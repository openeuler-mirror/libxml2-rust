...
* Returns -1 in case of error, the index in the stack otherwise
*/
fn nameNsPush(mut ctxt: xmlParserCtxtPtr,
 mut value: *const xmlChar,
 mut prefix: *const xmlChar,
 mut URI: *const xmlChar,
 mut line: libc::c_int, mut nsNr: libc::c_int)
 -> libc::c_int {
let mut current_block: u64;
...
tmp = xmlRealloc_safe((safe_ctxt).nameTab as
                                           *mut *mut xmlChar
                                           as
                                           *mut libc::c_void,
                                       ((safe_ctxt).nameMax as
                                           libc::c_ulong).wrapping_mul(::std::mem::size_of::<*const xmlChar>()
                                           as
                                           libc::c_ulong))
as *mut *const xmlChar;
...
tmp2 =
xmlRealloc_safe((safe_ctxt).pushTab
                                               as
                                               *mut *mut libc::c_void
                                               as
                                               *mut libc::c_void,
                                           ((safe_ctxt).nameMax
                                               as
                                               libc::c_ulong).wrapping_mul(::std::mem::size_of::<xmlStartTag>()
                                               as
                                               libc::c_ulong))
as *mut xmlStartTag;
if tmp2.is_null() {
...
(safe_ctxt).pushTab =
xmlMalloc_safe(((safe_ctxt).nameMax as
libc::c_ulong).wrapping_mul(::std::mem::size_of::<xmlStartTag>()
as
libc::c_ulong))
as *mut xmlStartTag;
...
tag = &mut *(*ctxt).pushTab.offset((safe_ctxt).nameNr as isize) as
*mut xmlStartTag;
(*tag).prefix = prefix;
(*tag).URI = URI;
(*tag).line = line;
...
pub fn namePush(mut ctxt: xmlParserCtxtPtr,
...
let mut tmp: *mut *const xmlChar = 0 as *mut *const xmlChar;
tmp =
xmlRealloc_safe((safe_ctxt).nameTab as
                                           *mut *mut xmlChar
                                           as
                                           *mut libc::c_void,
                                       (((safe_ctxt).nameMax *
...
pub fn namePop(mut ctxt: xmlParserCtxtPtr)
  -> *const xmlChar {
let mut ret: *const xmlChar = 0 as *const xmlChar;
...
(safe_ctxt).space =
&mut *(*ctxt).spaceTab.offset((safe_ctxt).spaceNr as isize) as
*mut libc::c_int;
}
...
xmlParserInputGrow_safe((safe_ctxt).input, 250 as libc::c_int);
if unsafe{(*(*ctxt).input).cur > (*(*ctxt).input).end ||
...
if unsafe{!(*(*ctxt).input).cur.is_null() &&
...
pub fn xmlSkipBlankChars(mut ctxt: xmlParserCtxtPtr)
            -> libc::c_int {
...
let mut expandPE: libc::c_int =
((safe_ctxt).external != 0 as libc::c_int ||
(safe_ctxt).inputNr != 1 as libc::c_int) as libc::c_int;
loop  {
if unsafe{*(*(*ctxt).input).cur as libc::c_int == 0x20 as libc::c_int ||
...
unsafe{(*(*(*ctxt).input).cur.offset(1 as libc::c_int as
    isize) as libc::c_int
    == 0x20 as libc::c_int ||
    0x9 as libc::c_int <=
        *(*(*ctxt).input).cur.offset(1 as libc::c_int
            as isize) as
            libc::c_int &&
        *(*(*ctxt).input).cur.offset(1 as libc::c_int
            as isize) as
            libc::c_int <= 0xa as libc::c_int ||
    *(*(*ctxt).input).cur.offset(1 as libc::c_int as
        isize) as
...
pub fn xmlPushInput(mut ctxt: xmlParserCtxtPtr,
...
*(*(*ctxt).input).cur.offset(1 as libc::c_int as isize) as
libc::c_int == '#' as i32 &&
*(*(*ctxt).input).cur.offset(2 as libc::c_int as isize) as
libc::c_int == 'x' as i32} {
unsafe{
...
if unsafe{(safe_ctxt).progressive == 0 as libc::c_int &&
((*(*ctxt).input).end.offset_from((*(*ctxt).input).cur)
    as libc::c_long) <
...
if unsafe{*(*(*ctxt).input).cur as libc::c_int >= '0' as i32 &&
...
*/
fn xmlParseStringCharRef(mut ctxt: xmlParserCtxtPtr,
...
val =
val * 16 as libc::c_int +
    (cur as libc::c_int - 'a' as i32) + 10 as libc::c_int
} else if cur as libc::c_int >= 'A' as i32 &&
cur as libc::c_int <= 'F' as i32 {
val =
val * 16 as libc::c_int +
...
unsafe{*ptr.offset(1 as libc::c_int as isize) as libc::c_int ==
'#' as i32} {
unsafe{
...
pub fn xmlParserHandlePEReference(mut ctxt:
                     xmlParserCtxtPtr) {
let mut safe_ctxt = unsafe { &mut *ctxt };
...
*/
if (safe_ctxt).external == 0 as libc::c_int &&
(safe_ctxt).inputNr == 1 as libc::c_int {
return
}
if unsafe{*(*(*ctxt).input).cur.offset(1 as libc::c_int as isize) as
...
pub fn xmlStringLenDecodeEntities(mut ctxt:
                     xmlParserCtxtPtr,
...
if c == '&' as i32 &&
unsafe{*str.offset(1 as libc::c_int as isize) as libc::c_int
== '#' as i32} {
...
nbchars =unsafe{
    (nbchars as
        libc::c_ulong).wrapping_add(xmlCopyCharMultiByte(&mut *buffer.offset(nbchars
        as
        isize),
                                                         val)
        as
...
if nbchars.wrapping_add(100 as libc::c_int as
libc::c_ulong) > buffer_size {
let mut tmp: *mut xmlChar = 0 as *mut xmlChar;
let mut new_size: size_t =
    buffer_size.wrapping_mul(2 as libc::c_int as
        libc::c_ulong).wrapping_add(100
        as
        libc::c_int
        as
        libc::c_ulong);
...
(safe_ctxt).nbentities =unsafe{
    (safe_ctxt).nbentities.wrapping_add(((*ent).checked /
        2 as libc::c_int) as libc::c_ulong)}
}
if !ent.is_null() &&
unsafe{(*ent).etype as libc::c_uint ==
...
if unsafe{!(*ent).content.is_null()} {
    ...
    if nbchars.wrapping_add(100 as libc::c_int as
        ...
        let mut new_size_0: size_t =
            buffer_size.wrapping_mul(2 as libc::c_int
                as
                libc::c_ulong).wrapping_add(100
                as
                libc::c_int
...
if rep.is_null() {
    ...
    while unsafe{*current} as libc::c_int != 0 as libc::c_int
    {
        ...
        if !(nbchars.wrapping_add(100 as libc::c_int
            as
            libc::c_ulong) >
            buffer_size) {
            continue ;
...
xmlParserEntityCheck(ctxt, 0 as libc::c_int as size_t,
                     ent, 0 as libc::c_int as size_t);
if !ent.is_null() {
(safe_ctxt).nbentities =unsafe{
    (safe_ctxt).nbentities.wrapping_add(((*ent).checked /
        2 as
...
* internal subset
*/
    if (safe_ctxt).options &
        XML_PARSE_NOENT as libc::c_int !=
        0 as libc::c_int ||
        (safe_ctxt).options &
            XML_PARSE_DTDVALID as libc::c_int !=
            0 as libc::c_int ||
...
(safe_ctxt).depth += 1;
rep = unsafe{ xmlStringDecodeEntities(ctxt, (*ent).content,
                                what,
                                0 as libc::c_int as
                                    xmlChar,
                                0 as libc::c_int as
...
if rep.is_null() {
    if unsafe{!(*ent).content.is_null()} {
        unsafe{
            *(*ent).content.offset(0 as libc::c_int as
                isize) =
                0 as libc::c_int as xmlChar
        }
    }
...
pub fn xmlStringDecodeEntities(mut ctxt: xmlParserCtxtPtr,
                  mut str: *const xmlChar,
                  mut what: libc::c_int,
                  mut end: xmlChar,
                  mut end2: xmlChar,
...
return xmlStringLenDecodeEntities(ctxt, str, xmlStrlen_safe(str), what, end,
                  end2, end3);
}
/* ***********************************************************************
*									*
...
*/
fn areBlanks(mut ctxt: xmlParserCtxtPtr,
mut str: *const xmlChar, mut len: libc::c_int,
mut blank_chars: libc::c_int) -> libc::c_int {
let mut i: libc::c_int = 0;
let mut ret: libc::c_int = 0;
...
*/
if unsafe{(*ctxt).space.is_null() || *(*ctxt).space == 1 as libc::c_int ||
...
i = 0 as libc::c_int;
while i < len {
if unsafe{!(*str.offset(i as isize) as libc::c_int == 0x20 as libc::c_int
||
0x9 as libc::c_int <=
...
if unsafe{(*(*ctxt).node).type_0 as libc::c_uint !=
...
pub fn xmlSplitQName(mut ctxt: xmlParserCtxtPtr,
...
buffer =
xmlMallocAtomic_safe((max as
libc::c_ulong).wrapping_mul(::std::mem::size_of::<xmlChar>()
as
libc::c_ulong))
as *mut xmlChar;
if buffer.is_null() {
...
memcpy_safe(buffer as *mut libc::c_void,
buf.as_mut_ptr() as *const libc::c_void, len as libc::c_ulong);
...
if c == 0 as libc::c_int {
return xmlStrndup_safe(b"\x00" as *const u8 as *const libc::c_char as
              *mut xmlChar, 0 as libc::c_int)
}
len = 0 as libc::c_int;
/*
...
(0x41 as libc::c_int <= first &&
first <= 0x5a as libc::c_int ||
0x61 as libc::c_int <= first &&
    first <= 0x7a as libc::c_int ||
0xc0 as libc::c_int <= first &&
    first <= 0xd6 as libc::c_int ||
0xd8 as libc::c_int <= first &&
    first <= 0xf6 as libc::c_int ||
0xf8 as libc::c_int <= first) as libc::c_int
} else {
unsafe{
xmlCharInRange_safe(first as libc::c_uint,
               unsafe{&xmlIsBaseCharGroup})
}
}) != 0 ||
...
*/
fn xmlIsNameStartChar(mut ctxt: xmlParserCtxtPtr,
...
fn xmlIsNameChar(mut ctxt: xmlParserCtxtPtr,
    mut c: libc::c_int) -> libc::c_int {
let mut safe_ctxt = unsafe { &mut *ctxt };
if (safe_ctxt).options & XML_PARSE_OLD10 as libc::c_int == 0 as libc::c_int {
/*
...
fn xmlParseNameComplex(mut ctxt: xmlParserCtxtPtr)
          -> *const xmlChar {
let mut safe_ctxt = unsafe { &mut *ctxt };
let mut len: libc::c_int = 0 as libc::c_int;
let mut l: libc::c_int = 0;
...
xmlCharInRange_safe(c as libc::c_uint,
               unsafe{&xmlIsDigitGroup})
}) != 0 || c == '.' as i32 || c == '-' as i32 ||
c == '_' as i32 || c == ':' as i32 ||
(if c < 0x100 as libc::c_int {
0 as libc::c_int
...
xmlCharInRange_safe(c as libc::c_uint,
               unsafe{&xmlIsCombiningGroup})
}) != 0 ||
(if c < 0x100 as libc::c_int {
(c == 0xb7 as libc::c_int) as libc::c_int
} else {
xmlCharInRange_safe(c as libc::c_uint,
               unsafe{&xmlIsExtenderGroup})
}) != 0) {
let fresh50 = count;
count = count + 1;
...
if unsafe{(*ctxt).instate as libc::c_int ==
XML_PARSER_EOF as libc::c_int} {
return 0 as *const xmlChar
}
}
...
*(*(*ctxt).input).cur.offset(-(1 as libc::c_int) as isize) as
libc::c_int == '\r' as i32} {
return unsafe{xmlDictLookup_safe((*ctxt).dict,
...
pub fn xmlParseName(mut ctxt: xmlParserCtxtPtr)
       -> *const xmlChar {
...
if unsafe{*in_0 as libc::c_int >= 0x61 as libc::c_int &&
...
while unsafe{*in_0 as libc::c_int >= 0x61 as libc::c_int &&
*in_0 as libc::c_int <= 0x7a as libc::c_int ||
*in_0 as libc::c_int >= 0x41 as libc::c_int &&
*in_0 as libc::c_int <= 0x5a as libc::c_int ||
...
if unsafe{*in_0 as libc::c_int > 0 as libc::c_int &&
(*in_0 as libc::c_int) < 0x80 as libc::c_int} {
count = unsafe{in_0.offset_from((*(*ctxt).input).cur) as
libc::c_long as libc::c_int};
if count > 50000 as libc::c_int &&
unsafe{(*ctxt).options & XML_PARSE_HUGE as libc::c_int ==
0 as libc::c_int} {
...
fn xmlParseNCNameComplex(mut ctxt: xmlParserCtxtPtr)
            -> *const xmlChar {
let mut len: libc::c_int = 0 as libc::c_int;
let mut l: libc::c_int = 0;
let mut c: libc::c_int = 0;
...
* Returns the Name parsed or NULL
*/
fn xmlParseNCName(mut ctxt: xmlParserCtxtPtr)
     -> *const xmlChar {
let mut in_0: *const xmlChar = 0 as *const xmlChar;
...
(*in_0 as libc::c_int) < 0x80 as libc::c_int} {
count =
unsafe{in_0.offset_from((*(*ctxt).input).cur) as
    libc::c_long as libc::c_int};
if count > 50000 as libc::c_int &&
unsafe{(*ctxt).options & XML_PARSE_HUGE as libc::c_int ==
    0 as libc::c_int} {
...
*/
fn xmlParseNameAndCompare(mut ctxt: xmlParserCtxtPtr,
...
in_0 = unsafe{(*(*ctxt).input).cur};
while unsafe{*in_0 as libc::c_int != 0 as libc::c_int &&
*in_0 as libc::c_int == *cmp as libc::c_int} {
unsafe{in_0 = in_0.offset(1);
cmp = cmp.offset(1);}
}
if unsafe{*cmp as libc::c_int == 0 as libc::c_int &&
(*in_0 as libc::c_int == '>' as i32 ||
...
/* success */
unsafe{(*(*ctxt).input).col =
((*(*ctxt).input).col as libc::c_long +
...
*/
fn xmlParseStringName(mut ctxt: xmlParserCtxtPtr,
         mut str: *mut *const xmlChar)
         -> *mut xmlChar {
let mut buf: [xmlChar; 105] = [0; 105];
...
len +=
unsafe{xmlCopyCharMultiByte(&mut *buf.as_mut_ptr().offset(len as isize),
             c)};
}
...
len +=
    unsafe{xmlCopyCharMultiByte(&mut *buffer.offset(len as
        isize),
                         c)}
}
unsafe{cur = cur.offset(l as isize);
...
pub fn xmlParseNmtoken(mut ctxt: xmlParserCtxtPtr)
          -> *mut xmlChar {
let mut buf: [xmlChar; 105] = [0; 105];
let mut len: libc::c_int = 0 as libc::c_int;
let mut l: libc::c_int = 0;
...
len +=
unsafe{xmlCopyCharMultiByte(&mut *buf.as_mut_ptr().offset(len as
isize),
                 c)};
}
if unsafe{*(*(*ctxt).input).cur as libc::c_int == '\n' as i32} {
...
pub fn xmlParseEntityValue(mut ctxt: xmlParserCtxtPtr,
    ...
    loop  {
        ...
        if unsafe{*cur as libc::c_int == '%' as i32 ||
            *cur as libc::c_int == '&' as i32 &&
                *cur.offset(1 as libc::c_int as isize)
                    as libc::c_int != '#' as i32} {
            let mut name: *mut xmlChar =
                0 as *mut xmlChar;
            let mut tmp_0: xmlChar = unsafe{*cur};
            let mut nameOk: libc::c_int =
                0 as libc::c_int;
            ...
            if nameOk == 0 as libc::c_int ||
                ...
                current_block = 1624980031832806685;
                break ;
            } else if unsafe{*cur as libc::c_int ==
                0 as libc::c_int} {
                current_block = 7158658067966855297;
                break ;
            }
...
*/
fn xmlParseAttValueComplex(mut ctxt: xmlParserCtxtPtr,
...
let mut ent: xmlEntityPtr = 0 as *mut xmlEntity;
let mut safe_ctxt = unsafe { &mut *ctxt };
if unsafe{*(*(*ctxt).input).cur.offset(0 as libc::c_int as isize) as libc::c_int
== '\"' as i32} {
(safe_ctxt).instate = XML_PARSER_ATTRIBUTE_VALUE;
limit = '\"' as i32 as xmlChar;
xmlNextChar_safe(ctxt);
} else if unsafe{*(*(*ctxt).input).cur.offset(0 as libc::c_int as isize) as
libc::c_int == '\'' as i32} {
limit = '\'' as i32 as xmlChar;
(safe_ctxt).instate = XML_PARSER_ATTRIBUTE_VALUE;
xmlNextChar_safe(ctxt);
...
buf =
xmlMallocAtomic_safe(buf_size) as
*mut xmlChar;
if buf.is_null() {
...
(0x100 as libc::c_int <= c &&
    c <= 0xd7ff as libc::c_int ||
    0xe000 as libc::c_int <= c &&
        c <= 0xfffd as libc::c_int ||
    0x10000 as libc::c_int <= c &&
        c <= 0x10ffff as libc::c_int) as
    libc::c_int
}) != 0 && c != '<' as i32 &&
(safe_ctxt).instate as libc::c_int !=
XML_PARSER_EOF as libc::c_int)} {
...
if unsafe{*(*(*ctxt).input).cur.offset(1 as libc::c_int as
    isize) as
    libc::c_int == '#' as i32} {
    let mut val: libc::c_int = xmlParseCharRef(ctxt);
    if val == '&' as i32 {
        if (safe_ctxt).replaceEntities != 0 {
            if len.wrapping_add(10 as libc::c_int as
                libc::c_ulong) >
                buf_size {
            ...
            unsafe{*buf.offset(fresh60 as isize) =
                '&' as i32 as xmlChar};
        } else {
            /*
...
*/
            ...
            len = len.wrapping_add(1);
            unsafe {
                *buf.offset(fresh61 as isize) =
                    '&' as i32 as xmlChar;
                let fresh62 = len;
                len = len.wrapping_add(1);
                *buf.offset(fresh62 as isize) =
                    '#' as i32 as xmlChar;
                let fresh63 = len;
                len = len.wrapping_add(1);
        ...
        if len.wrapping_add(10 as libc::c_int as
            ...
            let mut tmp_1: *mut xmlChar =
                0 as *mut xmlChar;
            let mut new_size_1: size_t =
                buf_size.wrapping_mul(2 as libc::c_int
                    as
                    libc::c_ulong).wrapping_add(10
                    as
                    libc::c_int
        ...
        len =
            unsafe{(len as
                libc::c_ulong).wrapping_add(xmlCopyChar(0
                                                            as
                                                            libc::c_int,
                                                        &mut *buf.offset(len
                                                            as
    ...
    (safe_ctxt).nbentities =
        (safe_ctxt).nbentities.wrapping_add(1);
    if !ent.is_null() {
        unsafe{(*ctxt).nbentities =
    ...
    if !ent.is_null() &&
        ...
        if unsafe{(*ctxt).replaceEntities == 0 as libc::c_int
            &&
            *(*ent).content.offset(0 as libc::c_int
                as isize) as
                libc::c_int == '&' as i32} {
            let fresh66 = len;
            len = len.wrapping_add(1);
        ...
        (safe_ctxt).replaceEntities !=
            0 as libc::c_int {
        if unsafe{(*ent).etype as libc::c_uint !=
            XML_INTERNAL_PREDEFINED_ENTITY as
                libc::c_int as libc::c_uint} {
            (safe_ctxt).depth += 1;
            rep =
                unsafe{xmlStringDecodeEntities(ctxt,
                                        (*ent).content,
                                        1 as
                                            libc::c_int,
                                        0 as
            ...
            if !rep.is_null() {
                current = rep;
                while unsafe{*current} as libc::c_int !=
                    0 as libc::c_int {
                    /* non input consuming */
                    if unsafe{*current} as libc::c_int ==
                        0xd as libc::c_int ||
            ...
            if  unsafe{!(*ent).content.is_null()} {
                let fresh75 = len;
                len = len.wrapping_add(1);
                unsafe{*buf.offset(fresh75 as isize) =
                    *(*ent).content.offset(0 as
                        libc::c_int
                        as
                        isize)};
...
*/
        if (safe_ent).etype as libc::c_uint !=
            ...
            (safe_ctxt).depth -= 1;
            diff =
                (safe_ctxt).nbentities.wrapping_sub(oldnbent).wrapping_add(1
                    as
                    libc::c_int
                    as
                    libc::c_ulong);
            ...
            (safe_ent).checked =
                diff.wrapping_mul(2 as libc::c_int as
                    libc::c_ulong)
                    as libc::c_int;
            if !rep.is_null() {
                if !xmlStrchr_safe(rep,
                              '<' as i32 as
                                  xmlChar).is_null() {
                    (safe_ent).checked |= 1 as libc::c_int
                }
                ...
                unsafe{*(*ent).content.offset(0 as
                    libc::c_int
                    as isize) =
                    0 as libc::c_int as xmlChar};
            }
...
*/
        let fresh76 = len;
        len = len.wrapping_add(1);
        unsafe{*buf.offset(fresh76 as isize) =
            '&' as i32 as xmlChar;}
        ...
        let fresh79 = len;
        len = len.wrapping_add(1);
        unsafe{*buf.offset(fresh79 as isize) =
            ';' as i32 as xmlChar};
    }
}
} else {
if c == 0x20 as libc::c_int || c == 0xd as libc::c_int
    ...
    if len != 0 as libc::c_int as libc::c_ulong ||
        ...
        if normalize == 0 || in_space == 0 {
            if l == 1 as libc::c_int {
                let fresh80 = len;
                len = len.wrapping_add(1);
                unsafe{*buf.offset(fresh80 as isize) =
                    0x20 as libc::c_int as xmlChar};
            } else {
                len =
                    unsafe{(len as
                        libc::c_ulong).wrapping_add(xmlCopyCharMultiByte(&mut *buf.offset(len
                        as
                        isize),
                                                                         0x20
                                                                             as
            ...
            while len.wrapping_add(10 as libc::c_int
                as
                libc::c_ulong)
                > buf_size {
    ...
    if len.wrapping_add(10 as libc::c_int as
        libc::c_ulong) > buf_size
    {
        let mut tmp_7: *mut xmlChar =
            0 as *mut xmlChar;
        let mut new_size_7: size_t =
            buf_size.wrapping_mul(2 as libc::c_int as
                libc::c_ulong).wrapping_add(10
                as
                libc::c_int
                as
...
if in_space != 0 && normalize != 0 {
    while len > 0 as libc::c_int as libc::c_ulong &&
        unsafe{*buf.offset(len.wrapping_sub(1 as
            libc::c_int
            as
            libc::c_ulong)
...
pub fn xmlParseAttValue(mut ctxt: xmlParserCtxtPtr)
           -> *mut xmlChar {
if ctxt.is_null() || unsafe{(*ctxt).input.is_null()} { return 0 as *mut xmlChar }
return unsafe{xmlParseAttValueInternal(ctxt, 0 as *mut libc::c_int,
...
pub fn xmlParseSystemLiteral(mut ctxt: xmlParserCtxtPtr)
                -> *mut xmlChar {
let mut buf: *mut xmlChar = 0 as *mut xmlChar;
let mut len: libc::c_int = 0 as libc::c_int;
let mut size: libc::c_int = 100 as libc::c_int;
...
let mut count: libc::c_int = 0 as libc::c_int;
if (safe_ctxt).progressive == 0 as libc::c_int &&
unsafe{(*(*ctxt).input).cur.offset_from((*(*ctxt).input).base) as
libc::c_long >
...
if (safe_ctxt).progressive == 0 as libc::c_int &&
unsafe{(*(*ctxt).input).cur.offset_from((*(*ctxt).input).base)
as libc::c_long >
(2 as libc::c_int * 250 as libc::c_int) as libc::c_long
&&
((*(*ctxt).input).end.offset_from((*(*ctxt).input).cur)
...
(0x9 as libc::c_int <= cur && cur <= 0xa as libc::c_int ||
cur == 0xd as libc::c_int || 0x20 as libc::c_int <= cur) as
libc::c_int
} else {
(0x100 as libc::c_int <= cur && cur <= 0xd7ff as libc::c_int ||
...
pub fn xmlParsePubidLiteral(mut ctxt: xmlParserCtxtPtr)
               -> *mut xmlChar {
let mut buf: *mut xmlChar = 0 as *mut xmlChar;
let mut len: libc::c_int = 0 as libc::c_int;
let mut size: libc::c_int = 100 as libc::c_int;
...
pub fn xmlParseCharData(mut ctxt: xmlParserCtxtPtr,
...
if nbchar > 0 as libc::c_int {
    ...
    if unsafe{!(*ctxt).sax.is_null() &&
        (*(*ctxt).sax).ignorableWhitespace !=
            (*(*ctxt).sax).characters} {
        if areBlanks(ctxt, tmp, nbchar, 1 as libc::c_int)
            != 0 {
...
unsafe {
    ...
    if *in_0 as libc::c_int == 0xa as libc::c_int {
        ...
        if *in_0.offset(1 as libc::c_int as isize) as
            libc::c_int == ']' as i32 &&
            *in_0.offset(2 as libc::c_int as isize) as
                libc::c_int == '>' as i32 {
...
if unsafe{!(*ctxt).sax.is_null() &&
    ...
    unsafe{(*(*ctxt).input).cur = in_0};
    if areBlanks(ctxt, tmp_0, nbchar, 0 as libc::c_int) !=
        0 {
        if unsafe{(*(*ctxt).sax).ignorableWhitespace.is_some()} {
            unsafe{
                (*(*ctxt).sax).ignorableWhitespace.expect("non-null function pointer")((*ctxt).userData,
...
17769492591016358583 => {
    ...
    if unsafe{(*ctxt).progressive == 0 as libc::c_int &&
        (*(*ctxt).input).cur.offset_from((*(*ctxt).input).base)
            as libc::c_long >
            (2 as libc::c_int * 250 as libc::c_int) as
                libc::c_long &&
        ((*(*ctxt).input).end.offset_from((*(*ctxt).input).cur)
            as libc::c_long) <
            (2 as libc::c_int * 250 as libc::c_int) as
...
if unsafe{!(*in_0 as libc::c_int >= 0x20 as libc::c_int &&
...
*/
fn xmlParseCharDataComplex(mut ctxt: xmlParserCtxtPtr,
              mut cdata: libc::c_int) {
...
(0x100 as libc::c_int <= cur &&
cur <= 0xd7ff as libc::c_int ||
0xe000 as libc::c_int <= cur &&
cur <= 0xfffd as libc::c_int ||
0x10000 as libc::c_int <= cur &&
cur <= 0x10ffff as libc::c_int) as libc::c_int
...
if cur == ']' as i32 &&
unsafe{*(*(*ctxt).input).cur.offset(1 as libc::c_int as isize) as
libc::c_int == ']' as i32 &&
*(*(*ctxt).input).cur.offset(2 as libc::c_int as isize) as
libc::c_int == '>' as i32} {
if cdata != 0 { break ; }
...
nbchar +=
unsafe{xmlCopyCharMultiByte(&mut *buf.as_mut_ptr().offset(nbchar as
isize),
                 cur)};
}
...
pub fn xmlParseExternalID(mut ctxt: xmlParserCtxtPtr,
...
if unsafe{*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(0 as libc::c_int as isize) as
libc::c_int == 'S' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(1 as libc::c_int as isize) as
libc::c_int == 'Y' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(2 as libc::c_int as isize) as
libc::c_int == 'S' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(3 as libc::c_int as isize) as
libc::c_int == 'T' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(4 as libc::c_int as isize) as
libc::c_int == 'E' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(5 as libc::c_int as isize) as
libc::c_int == 'M' as i32} {
unsafe{(*(*ctxt).input).cur =
...
} else if unsafe{ *((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(0 as libc::c_int as isize) as
libc::c_int == 'P' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(1 as libc::c_int as isize)
as libc::c_int == 'U' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(2 as libc::c_int as isize)
as libc::c_int == 'B' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(3 as libc::c_int as isize)
as libc::c_int == 'L' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(4 as libc::c_int as isize)
as libc::c_int == 'I' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(5 as libc::c_int as isize)
...
*/
fn xmlParseCommentComplex(mut ctxt: xmlParserCtxtPtr,
             mut buf: *mut xmlChar,
             mut len: size_t,
             mut size: size_t) {
let mut safe_ctxt  = unsafe{&mut *ctxt};
...
buf =
xmlMallocAtomic_safe(size.wrapping_mul(::std::mem::size_of::<xmlChar>()
as
libc::c_ulong))
as *mut xmlChar;
if buf.is_null() {
...
q == 0xd as libc::c_int || 0x20 as libc::c_int <= q) as
libc::c_int
} else {
...
0xe000 as libc::c_int <= r &&
    r <= 0xfffd as libc::c_int ||
0x10000 as libc::c_int <= r &&
    r <= 0x10ffff as libc::c_int) as libc::c_int
} == 0 {
unsafe{
xmlFatalErrMsgInt(ctxt, XML_ERR_INVALID_CHAR,
                  b"xmlParseComment: invalid xmlChar value %d\n\x00"
...
if !(cur == 0 as libc::c_int) {
while (if cur < 0x100 as libc::c_int {
(0x9 as libc::c_int <= cur &&
    cur <= 0xa as libc::c_int ||
    cur == 0xd as libc::c_int ||
    0x20 as libc::c_int <= cur) as libc::c_int
} else {
(0x100 as libc::c_int <= cur &&
...
if len.wrapping_add(5 as libc::c_int as libc::c_ulong) >=
    size {
...
xmlFatalErrMsgStr(ctxt, XML_ERR_COMMENT_NOT_FINISHED,
      b"Comment not terminated\n\x00" as *const u8 as
          *const libc::c_char, 0 as *const xmlChar);
}
xmlFree_safe(buf as *mut libc::c_void);
}
/* *
...
*(*(*ctxt).input).cur.offset(2 as libc::c_int as isize) as
libc::c_int != '-' as i32 ||
*(*(*ctxt).input).cur.offset(3 as libc::c_int as isize) as
libc::c_int != '-' as i32} {
return
}
...
if nbchar > 0 as libc::c_int as libc::c_ulong {
    if !(*ctxt).sax.is_null() &&
        ...
        if buf.is_null() {
            if *in_0 as libc::c_int == '-' as i32 &&
                *in_0.offset(1 as libc::c_int as isize) as
                    libc::c_int == '-' as i32 {
                size =
                    nbchar.wrapping_add(1 as libc::c_int as
                        libc::c_ulong)
            } else {
                size =
                    (100 as libc::c_int as
                        libc::c_ulong).wrapping_add(nbchar)
            }
            buf =
                xmlMallocAtomic_safe(size.wrapping_mul(::std::mem::size_of::<xmlChar>()
                    as
            ...
            size =
                (size as
                    libc::c_ulong).wrapping_add(len.wrapping_add(nbchar).wrapping_add(100
                    as
                    libc::c_int
                    as
                    libc::c_ulong))
        ...
        memcpy_safe(&mut *buf.offset(len as isize) as *mut xmlChar
                        as *mut libc::c_void,
                    (*(*ctxt).input).cur as *const libc::c_void,
                    nbchar);
        len =
            (len as libc::c_ulong).wrapping_add(nbchar) as
                size_t as size_t;
...
if *in_0.offset(1 as libc::c_int as isize) as libc::c_int ==
    ...
    if *in_0.offset(2 as libc::c_int as isize) as libc::c_int
        ...
        (*(*ctxt).input).cur =
            (*(*ctxt).input).cur.offset(3 as libc::c_int as
                isize);
        (*(*ctxt).input).col += 3 as libc::c_int;
        if *(*(*ctxt).input).cur as libc::c_int ==
            0 as libc::c_int {
...
pub fn xmlParsePITarget(mut ctxt: xmlParserCtxtPtr)
           -> *const xmlChar {
let mut name: *const xmlChar = 0 as *const xmlChar;
...
unsafe{
if *name.offset(3 as libc::c_int as isize) as libc::c_int ==
0 as libc::c_int {
...
fn xmlParseCatalogPI(mut ctxt: xmlParserCtxtPtr,
...
if !(xmlStrncmp_safe(tmp,
b"catalog\x00" as *const u8 as *const libc::c_char as
    *mut xmlChar, 7 as libc::c_int) != 0) {
unsafe{
...
if !(marker as libc::c_int != '\'' as i32 &&
marker as libc::c_int != '\"' as i32) {
unsafe{
tmp = tmp.offset(1);
base = tmp;
...
if !URL.is_null() {
    (safe_ctxt).catalogs =
        xmlCatalogAddLocal_safe((safe_ctxt).catalogs, URL);
    xmlFree_safe(URL as
        *mut libc::c_void);
}
return
...
xmlWarningMsg(ctxt, XML_WAR_CATALOG_PI,
  b"Catalog PI syntax error: %s\n\x00" as *const u8 as
      *const libc::c_char, catalog, 0 as *const xmlChar);
}
if !URL.is_null() {
...
#[cfg(HAVE_parser_LIBXML_CATALOG_ENABLED)] _ => {
    if (state as libc::c_int == XML_PARSER_MISC as libc::c_int ||
        state as libc::c_int ==
            XML_PARSER_START as libc::c_int) &&
        xmlStrEqual_safe(target,
        ...
        if allow as libc::c_uint ==
            XML_CATA_ALLOW_DOCUMENT as libc::c_int as
                libc::c_uint ||
            allow as libc::c_uint ==
                XML_CATA_ALLOW_ALL as libc::c_int as
                    libc::c_uint {
...
libc::c_int == 'A' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(6 as libc::c_int as isize) as
libc::c_int == 'T' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(7 as libc::c_int as isize) as
libc::c_int == 'I' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(8 as libc::c_int as isize) as
libc::c_int == 'O' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(9 as libc::c_int as isize) as
libc::c_int == 'N' as i32} {
let mut inputid: libc::c_int = unsafe{(*(*ctxt).input).id};
if unsafe{(*ctxt).progressive == 0 as libc::c_int &&
(*(*ctxt).input).cur.offset_from((*(*ctxt).input).base)
...
unsafe{
(*(*ctxt).input).cur =
(*(*ctxt).input).cur.offset(10 as libc::c_int as isize);
...
URI =
    xmlParseExternalID(ctxt, &mut literal, 1 as libc::c_int);
...
xmlStrEqual_safe((*(*ctxt).myDoc).version,
                 b"SAX compatibility mode document\x00" as
                     *const u8 as *const libc::c_char as
                     *mut xmlChar) != 0 {
if (*ctxt).myDoc.is_null() {
    (*ctxt).myDoc =
        xmlNewDoc_safe(b"SAX compatibility mode document\x00" as
            *const u8 as *const libc::c_char as
            *mut xmlChar);
    if (*ctxt).myDoc.is_null() {
        xmlErrMemory(ctxt,
    ...
    (*(*ctxt).myDoc).properties =
        XML_DOC_INTERNAL as libc::c_int
}
...
xmlSAX2EntityDecl(ctxt as *mut libc::c_void, name,
...
if (*ctxt).replaceEntities != 0 as libc::c_int &&
    ...
    if (*ctxt).myDoc.is_null() {
        (*ctxt).myDoc =
            xmlNewDoc_safe(b"SAX compatibility mode document\x00"
                as *const u8 as *const libc::c_char
                as *mut xmlChar);
        if (*ctxt).myDoc.is_null() {
            xmlErrMemory(ctxt,
...
if isParameter != 0 {
    ...
    if cur.is_null() &&
        ...
        cur =
            xmlSAX2GetEntity_safe(ctxt as *mut libc::c_void, name)
    }
}
...
if !literal.is_null() {
xmlFree_safe(literal as
*mut libc::c_void);
}
if !orig.is_null() {
xmlFree_safe(orig as
*mut libc::c_void);
}
};
}
...
pub fn xmlParseDefaultDecl(mut ctxt: xmlParserCtxtPtr,
...
pub fn xmlParseNotationType(mut ctxt: xmlParserCtxtPtr)
               -> xmlEnumerationPtr {
let mut name: *const xmlChar = 0 as *const xmlChar;
...
pub fn xmlParseEnumerationType(mut ctxt: xmlParserCtxtPtr)
                  -> xmlEnumerationPtr {
...
pub fn xmlParseEnumeratedType(mut ctxt: xmlParserCtxtPtr,
                 mut tree:
...
pub fn xmlParseAttributeType(mut ctxt: xmlParserCtxtPtr,
                mut tree:
                *mut xmlEnumerationPtr)
                -> libc::c_int {
//@todo 削减unsafe范围
...
if *(*(*ctxt).input).cur as libc::c_int == 'I' as i32 &&
    *(*(*ctxt).input).cur.offset(1 as libc::c_int as isize)
        as libc::c_int == 'D' as i32 {
    (*(*ctxt).input).cur =
        (*(*ctxt).input).cur.offset(2 as libc::c_int as
            isize);
    (*(*ctxt).input).col += 2 as libc::c_int;
    if *(*(*ctxt).input).cur as libc::c_int ==
        0 as libc::c_int {
    ...
    if *((*(*ctxt).input).cur as
        *mut libc::c_uchar).offset(0 as libc::c_int as
        isize) as
        libc::c_int == 'E' as i32 &&
        *((*(*ctxt).input).cur as
            *mut libc::c_uchar).offset(1 as libc::c_int
            as isize) as
            libc::c_int == 'N' as i32 &&
        *((*(*ctxt).input).cur as
            *mut libc::c_uchar).offset(2 as libc::c_int
            as isize) as
            libc::c_int == 'T' as i32 &&
        *((*(*ctxt).input).cur as
            *mut libc::c_uchar).offset(3 as libc::c_int
            as isize) as
            libc::c_int == 'I' as i32 &&
        *((*(*ctxt).input).cur as
            *mut libc::c_uchar).offset(4 as libc::c_int
            ...
            libc::c_int == 'T' as i32 &&
        *((*(*ctxt).input).cur as
            *mut libc::c_uchar).offset(5 as libc::c_int
            as isize) as
            libc::c_int == 'Y' as i32 {
        (*(*ctxt).input).cur =
            (*(*ctxt).input).cur.offset(6 as libc::c_int as
                isize);
        (*(*ctxt).input).col += 6 as libc::c_int;
        if *(*(*ctxt).input).cur as libc::c_int ==
            0 as libc::c_int {
        ...
        if *((*(*ctxt).input).cur as
            *mut libc::c_uchar).offset(0 as libc::c_int
            as isize) as
            libc::c_int == 'E' as i32 &&
            *((*(*ctxt).input).cur as
                *mut libc::c_uchar).offset(1 as
                libc::c_int
                as isize)
            ...
            *((*(*ctxt).input).cur as
                *mut libc::c_uchar).offset(4 as
                ...
                as libc::c_int == 'T' as i32 &&
            *((*(*ctxt).input).cur as
                *mut libc::c_uchar).offset(5 as
                libc::c_int
                as isize)
                as libc::c_int == 'I' as i32 &&
            *((*(*ctxt).input).cur as
                *mut libc::c_uchar).offset(6 as
                libc::c_int
                as isize)
                as libc::c_int == 'E' as i32 &&
            *((*(*ctxt).input).cur as
                *mut libc::c_uchar).offset(7 as
                libc::c_int
                as isize)
                as libc::c_int == 'S' as i32 {
            (*(*ctxt).input).cur =
                (*(*ctxt).input).cur.offset(8 as libc::c_int
                    as isize);
            (*(*ctxt).input).col += 8 as libc::c_int;
            if *(*(*ctxt).input).cur as libc::c_int ==
                0 as libc::c_int {
...
pub fn xmlParseAttributeListDecl(mut ctxt:
                    xmlParserCtxtPtr) {
    ...
    if xmlSkipBlankChars(ctxt) == 0 as libc::c_int {
        ...
        if def <= 0 as libc::c_int {
            ...
            if (*ctxt).sax2 != 0 && !defaultValue.is_null() &&
                ...
                &&
                def !=
                    XML_ATTRIBUTE_REQUIRED as libc::c_int {
...
pub fn xmlParseElementMixedContentDecl(mut ctxt:
                          xmlParserCtxtPtr,
                          mut inputchk:
                          libc::c_int)
...
xmlNextChar_safe(ctxt);
ret =
xmlNewDocElementContent_safe((*ctxt).myDoc, 0 as *const xmlChar,
...
if elem.is_null() {
ret =
    xmlNewDocElementContent_safe((*ctxt).myDoc,
                            0 as *const xmlChar,
...
(*n).c1 =
    xmlNewDocElementContent_safe((*ctxt).myDoc, elem,
...
*          hierarchy.
*/
fn xmlParseElementChildrenContentDeclPriv(mut ctxt:
                             xmlParserCtxtPtr,
                             mut inputchk:
                             libc::c_int,
                             mut depth:
...
ret =
xmlNewDocElementContent_safe((safe_ctxt).myDoc, elem,
                XML_ELEMENT_CONTENT_ELEMENT);
cur = ret;
if cur.is_null() {
unsafe{
...
xmlNextChar_safe(ctxt);
op =
xmlNewDocElementContent_safe((safe_ctxt).myDoc, 0 as *const xmlChar,
...
(*cur).type_0 as libc::c_uint ==
    XML_ELEMENT_CONTENT_OR as libc::c_int as
        libc::c_uint {
if !(*cur).c1.is_null() &&
    ((*(*cur).c1).ocur as libc::c_uint ==
        XML_ELEMENT_CONTENT_OPT as libc::c_int as
            libc::c_uint ||
        (*(*cur).c1).ocur as libc::c_uint ==
            XML_ELEMENT_CONTENT_MULT as libc::c_int as
                libc::c_uint) {
...
pub fn xmlParseElementChildrenContentDecl(mut ctxt:
                             xmlParserCtxtPtr,
                             mut inputchk:
                             libc::c_int)
                             -> xmlElementContentPtr {
...
pub fn xmlParseElementContentDecl(mut ctxt:
                     xmlParserCtxtPtr,
                     mut name: *const xmlChar,
                     mut result:
...
pub fn xmlParseElementDecl(mut ctxt: xmlParserCtxtPtr)
              -> libc::c_int {
let mut name: *const xmlChar = 0 as *const xmlChar;
...
xmlSkipBlankChars(ctxt);
if *((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(0 as libc::c_int as isize)
as libc::c_int == 'I' as i32 &&
*((*(*ctxt).input).cur as
    *mut libc::c_uchar).offset(1 as libc::c_int as
    isize) as
    libc::c_int == 'N' as i32 &&
*((*(*ctxt).input).cur as
    *mut libc::c_uchar).offset(2 as libc::c_int as
    isize) as
    libc::c_int == 'C' as i32 &&
*((*(*ctxt).input).cur as
    *mut libc::c_uchar).offset(3 as libc::c_int as
    isize) as
    libc::c_int == 'L' as i32 &&
*((*(*ctxt).input).cur as
    *mut libc::c_uchar).offset(4 as libc::c_int as
    isize) as
    libc::c_int == 'U' as i32 &&
*((*(*ctxt).input).cur as
    *mut libc::c_uchar).offset(5 as libc::c_int as
    isize) as
    libc::c_int == 'D' as i32 &&
*((*(*ctxt).input).cur as
    *mut libc::c_uchar).offset(6 as libc::c_int as
    isize) as
    libc::c_int == 'E' as i32 {
(*(*ctxt).input).cur =
    (*(*ctxt).input).cur.offset(7 as libc::c_int as
        isize);
(*(*ctxt).input).col += 7 as libc::c_int;
if *(*(*ctxt).input).cur as libc::c_int ==
    0 as libc::c_int {
    ...
    if inputIdsSize <= depth {
        ...
        inputIdsSize =
            if inputIdsSize ==
                ...
                inputIdsSize.wrapping_mul(2 as libc::c_int
                    as
                    libc::c_ulong)
            };
        tmp =
            xmlRealloc_safe(inputIds
                                as
                                *mut libc::c_void,
                            inputIdsSize.wrapping_mul(::std::mem::size_of::<libc::c_int>()
                                as
                                libc::c_ulong))
                as *mut libc::c_int;
        if tmp.is_null() {
 ...
 */
    ...
    while *(*(*ctxt).input).cur as libc::c_int !=
        0 as libc::c_int {
        if *(*(*ctxt).input).cur as libc::c_int ==
            '<' as i32 &&
    ...
    if *(*(*ctxt).input).cur as libc::c_int ==
        ...
        (*(*ctxt).input).cur =
            (*(*ctxt).input).cur.offset(3 as libc::c_int
                as isize);
        (*(*ctxt).input).col += 3 as libc::c_int;
        if *(*(*ctxt).input).cur as libc::c_int ==
            0 as libc::c_int {
...
let mut cons: libc::c_uint =
(*(*ctxt).input).consumed as libc::c_uint;
...
69 => {
    ...
    } else if *(*(*ctxt).input).cur.offset(3 as libc::c_int as
        isize) as
        libc::c_int == 'N' as i32 {
...
} else if *(*(*ctxt).input).cur.offset(1 as libc::c_int as isize) as
libc::c_int == '?' as i32 {
xmlParsePI(ctxt);
}
}
...
*mut libc::c_uchar).offset(4 as libc::c_int as isize) as
libc::c_int == 'l' as i32 &&
(*(*(*ctxt).input).cur.offset(5 as libc::c_int as isize) as
libc::c_int == 0x20 as libc::c_int ||
0x9 as libc::c_int <=
*(*(*ctxt).input).cur.offset(5 as libc::c_int as isize) as
    libc::c_int &&
...
pub fn xmlParseExternalSubset(mut ctxt: xmlParserCtxtPtr,
                 mut ExternalID:
                 *const xmlChar,
                 mut SystemID:
                 *const xmlChar) {
...
if (safe_ctxt).myDoc.is_null() {
(safe_ctxt).myDoc =
xmlNewDoc_safe(b"1.0\x00" as *const u8 as *const libc::c_char as
*mut xmlChar);
if (safe_ctxt).myDoc.is_null() {
unsafe{
xmlErrMemory(ctxt,
...
*/
if unsafe{*(*(*ctxt).input).cur.offset(1 as libc::c_int as isize) as libc::c_int
== '#' as i32} {
...
if hex == 'x' as i32 || hex == 'X' as i32 {
    snprintf(out.as_mut_ptr() as *mut libc::c_char,
             ::std::mem::size_of::<[xmlChar; 16]>() as
                 libc::c_ulong,
...
i += unsafe{xmlCopyCharMultiByte(&mut *out.as_mut_ptr().offset(i as
    isize), value)};
}
...
*/
unsafe{
if !(safe_ctxt).sax.is_null() && (*(*ctxt).sax).characters.is_some() &&
(safe_ctxt).disableSAX == 0 {
(*(*ctxt).sax).characters.expect("non-null function pointer")((safe_ctxt).userData,
                                                          val,
                                                          xmlStrlen_safe(val));
}
}
...
((safe_ent).etype as libc::c_uint !=
XML_EXTERNAL_GENERAL_PARSED_ENTITY as libc::c_int as
libc::c_uint ||
(safe_ctxt).options &
(XML_PARSE_NOENT as libc::c_int |
    XML_PARSE_DTDVALID as libc::c_int) != 0) {
let mut oldnbent: libc::c_ulong = (safe_ctxt).nbentities;
let mut diff: libc::c_ulong = 0;
/*
...
(safe_ctxt).depth += 1;
ret =
unsafe{xmlParseBalancedChunkMemoryInternal(ctxt, (safe_ent).content,
                                    user_data, &mut list)};
(safe_ctxt).depth -= 1
...
(safe_ctxt).depth += 1;
ret =
unsafe{xmlParseExternalEntityPrivate((safe_ctxt).myDoc, ctxt,
                              (safe_ctxt).sax, user_data,
                              (safe_ctxt).depth, (safe_ent).URI,
                              (safe_ent).ExternalID, &mut list)};
(safe_ctxt).depth -= 1
} else {
...
if xmlParserEntityCheck(ctxt, 0 as libc::c_int as size_t, ent,
...
(safe_ent).etype as libc::c_uint ==
    XML_EXTERNAL_GENERAL_PARSED_ENTITY as libc::c_int as
        libc::c_uint) && (safe_ent).children.is_null() {
(safe_ent).children = list;
/*
 * Prune it directly in the generated document
 * except for single text nodes.
 */
if unsafe{(safe_ctxt).replaceEntities == 0 as libc::c_int ||
...
ret as libc::c_uint !=
XML_WAR_UNDECLARED_ENTITY as libc::c_int as
    libc::c_uint {
unsafe{
...
(safe_ctxt).nbentities =
(safe_ctxt).nbentities.wrapping_add(((safe_ent).checked /
2 as libc::c_int) as
libc::c_ulong)
}
/*
...
XML_ERR_ENTITY_LOOP as libc::c_int as libc::c_uint {
unsafe{
...
*/
unsafe{
(*(*ctxt).sax).reference.expect("non-null function pointer")((safe_ctxt).userData,
                                                             (safe_ent).name);
}
}
return
...
(safe_ctxt).sizeentcopy.wrapping_add(((safe_ent).length +
    5 as libc::c_int) as
    libc::c_ulong);
if xmlParserEntityCheck(ctxt, 0 as libc::c_int as size_t, ent,
    ...
    unsafe{
        if (safe_ctxt).parseMode as libc::c_uint ==
            ...
            (*nw).type_0 as libc::c_uint ==
                XML_ELEMENT_NODE as libc::c_int as
                    libc::c_uint &&
            (*nw).children.is_null() {
...
unsafe{
    ...
    nw_0 =
        xmlDocCopyNode_safe(cur_0, (safe_ctxt).myDoc,
                            1 as libc::c_int);
    if !nw_0.is_null() {
...
pub fn xmlParseEntityRef(mut ctxt: xmlParserCtxtPtr)
            -> xmlEntityPtr {
let mut name: *const xmlChar = 0 as *const xmlChar;
...
ent = (*(*ctxt).sax).getEntity.expect("non-null function pointer")((safe_ctxt).userData,
                                                               name)
}
}
...
if (safe_ctxt).inSubset == 0 as libc::c_int && !(safe_ctxt).sax.is_null()
&& (*(*ctxt).sax).reference.is_some() {
(*(*ctxt).sax).reference.expect("non-null function pointer")((safe_ctxt).userData,
                                                             name);
}
...
XML_PARSER_ATTRIBUTE_VALUE as libc::c_int && !ent.is_null()
&&
(safe_ent).etype as libc::c_uint !=
XML_INTERNAL_PREDEFINED_ENTITY as libc::c_int as
libc::c_uint {
if ((safe_ent).checked & 1 as libc::c_int != 0 ||
(safe_ent).checked == 0 as libc::c_int) &&
!(safe_ent).content.is_null() &&
!xmlStrchr_safe((safe_ent).content, '<' as i32 as xmlChar).is_null() {
unsafe{
...
* is updated to the current location in the string.
*/
fn xmlParseStringEntityRef(mut ctxt: xmlParserCtxtPtr,
              mut str: *mut *const xmlChar)
              -> xmlEntityPtr {
let mut name: *mut xmlChar = 0 as *mut xmlChar;
let mut ptr: *const xmlChar = 0 as *const xmlChar;
...
xmlParserEntityCheck(ctxt, 0 as libc::c_int as size_t, ent,
         0 as libc::c_int as size_t);
/* TODO ? check regressions ctxt->valid = 0; */
} else if (safe_ent).etype as libc::c_uint ==
XML_EXTERNAL_GENERAL_UNPARSED_ENTITY as libc::c_int as
libc::c_uint {
unsafe{
xmlFatalErrMsgStr(ctxt, XML_ERR_UNPARSED_ENTITY,
          b"Entity reference to unparsed entity %s\n\x00" as
...
entity =
(*(*ctxt).sax).getParameterEntity.expect("non-null function pointer")((safe_ctxt).userData,
                                                                  name)
}
}
if (safe_ctxt).instate as libc::c_int == XML_PARSER_EOF as libc::c_int {
...
xmlParserEntityCheck(ctxt, 0 as libc::c_int as size_t,
         0 as xmlEntityPtr, 0 as libc::c_int as size_t);
...
XML_EXTERNAL_PARAMETER_ENTITY as libc::c_int as
libc::c_uint {
unsafe{
xmlWarningMsg(ctxt, XML_WAR_UNDECLARED_ENTITY,
...
let mut start: [xmlChar; 4] = [0; 4];
let mut enc: xmlCharEncoding = XML_CHAR_ENCODING_NONE;
if xmlParserEntityCheck(ctxt, 0 as libc::c_int as size_t, entity,
            0 as libc::c_int as size_t) != 0 {
return
}
...
start[3 as libc::c_int as usize] =
    *(*(*ctxt).input).cur.offset(3 as libc::c_int as isize);
enc =
    xmlDetectCharEncoding_safe(start.as_mut_ptr(),
                               4 as libc::c_int);
if enc as libc::c_int != XML_CHAR_ENCODING_NONE as libc::c_int
{
    xmlSwitchEncoding_safe(ctxt, enc);
...
(*(*(*ctxt).input).cur.offset(5 as libc::c_int as isize) as
    libc::c_int == 0x20 as libc::c_int ||
    0x9 as libc::c_int <=
        *(*(*ctxt).input).cur.offset(5 as libc::c_int as
            isize) as
            libc::c_int &&
        *(*(*ctxt).input).cur.offset(5 as libc::c_int as
            isize) as
...
*/
fn xmlLoadEntityContent(mut ctxt: xmlParserCtxtPtr,
...
(safe_entity).etype as libc::c_uint !=
XML_EXTERNAL_GENERAL_PARSED_ENTITY as libc::c_int as
libc::c_uint || !(safe_entity).content.is_null() {
unsafe{
xmlFatalErr(ctxt, XML_ERR_INTERNAL_ERROR,
...
*/
fn xmlParseStringPEReference(mut ctxt: xmlParserCtxtPtr,
                mut str: *mut *const xmlChar)
                -> xmlEntityPtr {
let mut ptr: *const xmlChar = 0 as *const xmlChar;
let mut cur: xmlChar = 0;
...
xmlWarningMsg(ctxt, XML_WAR_UNDECLARED_ENTITY,
      b"%%%s; is not a parameter entity\n\x00" as *const u8 as
          *const libc::c_char, name, 0 as *const xmlChar);
}
}
(safe_ctxt).hasPErefs = 1 as libc::c_int;
xmlFree_safe(name as *mut libc::c_void);
...
*/
unsafe{
if !(safe_ctxt).sax.is_null() && (*(*ctxt).sax).internalSubset.is_some() &&
(safe_ctxt).disableSAX == 0 {
(*(*ctxt).sax).internalSubset.expect("non-null function pointer")((safe_ctxt).userData,
                                                          name,
                                                          ExternalID,
                                                          URI);
}
...
pub fn xmlParseAttribute(mut ctxt: xmlParserCtxtPtr,
            mut value: *mut *mut xmlChar)
            -> *const xmlChar {
let mut name: *const xmlChar = 0 as *const xmlChar;
let mut val: *mut xmlChar = 0 as *mut xmlChar;
...
xmlStrEqual_safe(name,
b"xml:lang\x00" as *const u8 as *const libc::c_char as
    *mut xmlChar) != 0 {
if xmlCheckLanguageID(val) == 0 {
unsafe{
xmlWarningMsg(ctxt, XML_WAR_LANG_VALUE,
          b"Malformed value for xml:lang : %s\n\x00" as
              *const u8 as *const libc::c_char, val,
          0 as *const xmlChar);
}
}
}
...
b"xml:space\x00" as *const u8 as *const libc::c_char as
   *mut xmlChar) != 0 {
if xmlStrEqual_safe(val,
   b"default\x00" as *const u8 as *const libc::c_char as
       *mut xmlChar) != 0 {
unsafe{*(*ctxt).space = 0 as libc::c_int}
} else if xmlStrEqual_safe(val,
          b"preserve\x00" as *const u8 as
              *const libc::c_char as *mut xmlChar) != 0 {
...
*/
#[cfg(HAVE_parser_LIBXML_SAX1_ENABLED)]
pub fn xmlParseStartTag(mut ctxt: xmlParserCtxtPtr)
           -> *const xmlChar {
let mut safe_ctxt  = unsafe{&mut *ctxt};
let mut current_block: u64;
let mut name: *const xmlChar = 0 as *const xmlChar;
...
*/
{
    if atts.is_null() {
        maxatts =
            22 as
                libc::c_int; /* allow for 10 attrs by default */
        atts =
            xmlMalloc_safe((maxatts
                as
                libc::c_ulong).wrapping_mul(::std::mem::size_of::<*mut xmlChar>()
    ...
    match current_block {
        ...
        _ => {
            unsafe{
                ...
                nbatts = nbatts + 1;
                let ref mut fresh90 =
                    *atts.offset(fresh89 as isize);
                *fresh90 = attname;
                let fresh91 = nbatts;
                nbatts = nbatts + 1;
                let ref mut fresh92 =
                    *atts.offset(fresh91 as isize);
                *fresh92 = attvalue;
                let ref mut fresh93 =
                    *atts.offset(nbatts as isize);
                *fresh93 = 0 as *const xmlChar;
                let ref mut fresh94 =
                    *atts.offset((nbatts + 1 as libc::c_int) as
                        isize);
...
*/
if !(safe_ctxt).sax.is_null() && unsafe{(*(*ctxt).sax).startElement.is_some()} &&
(safe_ctxt).disableSAX == 0 {
if nbatts > 0 as libc::c_int {
unsafe{
(*(*ctxt).sax).startElement.expect("non-null function pointer")((safe_ctxt).userData,
                                                            name,
                                                            atts);
}
} else {
...
*/
#[cfg(HAVE_parser_LIBXML_SAX1_ENABLED)]
fn xmlParseEndTag1(mut ctxt: xmlParserCtxtPtr,
      mut line: libc::c_int) {
let mut name: *const xmlChar = 0 as *const xmlChar;
//@todo 削减unsafe范围
unsafe{
...
if name.is_null() {
name =
b"unparsable\x00" as *const u8 as *const libc::c_char as
    *mut xmlChar
}
...
*/
fn xmlGetNamespace(mut ctxt: xmlParserCtxtPtr,
      mut prefix: *const xmlChar)
      -> *const xmlChar {
let mut i: libc::c_int = 0;
let mut safe_ctxt  = unsafe{&mut *ctxt};
...
if prefix.is_null() &&
**(*ctxt).nsTab.offset((i + 1 as libc::c_int) as isize) as
    libc::c_int == 0 as libc::c_int {
return 0 as *const xmlChar
}
...
* Returns the Name parsed or NULL
*/
fn xmlParseQName(mut ctxt: xmlParserCtxtPtr,
...
xmlNsErr(ctxt, XML_NS_ERR_QNAME,
         b"Failed to parse QName \'%s\'\n\x00" as *const u8 as
             *const libc::c_char, l, 0 as *const xmlChar,
         0 as *const xmlChar);
*prefix = 0 as *const xmlChar;
return l
}
...
xmlNsErr(ctxt, XML_NS_ERR_QNAME,
 b"Failed to parse QName \'%s:\'\n\x00" as *const u8 as
     *const libc::c_char, p, 0 as *const xmlChar,
 0 as *const xmlChar);
...
tmp =
xmlBuildQName_safe(b"\x00" as *const u8 as *const libc::c_char
                  as *mut xmlChar, p, 0 as *mut xmlChar,
              0 as libc::c_int)
} else {
tmp =
xmlBuildQName_safe(l, p, 0 as *mut xmlChar, 0 as libc::c_int);
xmlFree_safe(l as
*mut libc::c_char
as
*mut libc::c_void);
...
xmlNsErr(ctxt, XML_NS_ERR_QNAME,
 b"Failed to parse QName \'%s:%s:\'\n\x00" as *const u8 as
     *const libc::c_char, p, l, 0 as *const xmlChar);
xmlNextChar_safe(ctxt);
tmp_0 = xmlParseName(ctxt) as *mut xmlChar;
if !tmp_0.is_null() {
tmp_0 =
xmlBuildQName_safe(tmp_0, l, 0 as *mut xmlChar,
              0 as libc::c_int);
l = xmlDictLookup_safe((safe_ctxt).dict, tmp_0, -(1 as libc::c_int));
if !tmp_0.is_null() {
xmlFree_safe(tmp_0 as
    *mut libc::c_void);
}
unsafe{*prefix = p;}
return l
...
tmp_0 =
xmlBuildQName_safe(b"\x00" as *const u8 as *const libc::c_char as
...
* and the name for mismatch
*/
fn xmlParseQNameAndCompare(mut ctxt: xmlParserCtxtPtr,
              mut name: *const xmlChar,
              mut prefix: *const xmlChar)
              -> *const xmlChar {
...
*/
fn xmlParseAttValueInternal(mut ctxt: xmlParserCtxtPtr,
               mut len: *mut libc::c_int,
...
*/
while in_0 < end && unsafe{*in_0} as libc::c_int != limit as libc::c_int &&
...
* Returns the attribute name, and the value in *value, .
*/
fn xmlParseAttribute2(mut ctxt: xmlParserCtxtPtr,
...
let mut type_0: libc::c_int = 0;
type_0 =
xmlHashQLookup2_safe((safe_ctxt).attsSpecial, pref, elem, unsafe{*prefix}, name) as
ptrdiff_t as libc::c_int;
...
if (safe_ctxt).pedantic != 0 &&
xmlStrEqual_safe(name,
    b"lang\x00" as *const u8 as *const libc::c_char as
        *mut xmlChar) != 0 {
...
if xmlStrEqual_safe(name,
   b"space\x00" as *const u8 as *const libc::c_char as
       *mut xmlChar) != 0 {
internal_val = xmlStrndup_safe(val, unsafe{*len});
if xmlStrEqual_safe(internal_val,
       b"default\x00" as *const u8 as *const libc::c_char
           as *mut xmlChar) != 0 {
unsafe{*(*ctxt).space = 0 as libc::c_int}
} else if xmlStrEqual_safe(internal_val,
              b"preserve\x00" as *const u8 as
                  *const libc::c_char as *mut xmlChar) !=
0 {
unsafe{*(*ctxt).space = 1 as libc::c_int}
...
* Returns the element name parsed
*/
fn xmlParseStartTag2(mut ctxt: xmlParserCtxtPtr,
...
*tlen =
((*(*ctxt).input).cur.offset_from((*(*ctxt).input).base) as
libc::c_long as libc::c_ulong).wrapping_sub(cur) as libc::c_int;
}
/*
* Now parse the attributes, it ends up with the ending
*
...
xmlDictLookup_safe((safe_ctxt).dict, attvalue, len);
let mut uri: xmlURIPtr = 0 as *mut xmlURI;
if URL.is_null() {
unsafe{
    xmlErrMemory(ctxt,
...
*/
        j = 1 as libc::c_int;
        while j <= nbNs {
            if unsafe{(*(*ctxt).nsTab.offset(((*ctxt).nsNr -
                2 as
                    libc::c_int
                    * j) as
                isize)).is_null()}
...
unsafe{*URL_0.offset(0 as libc::c_int as isize) as
    libc::c_int == 0 as libc::c_int} {
xmlNsErr(ctxt, XML_NS_ERR_XML_NAMESPACE,
         b"xmlns:%s: Empty XML namespace is not allowed\n\x00"
             as *const u8 as *const libc::c_char, attname,
         0 as *const xmlChar, 0 as *const xmlChar);
} else {
...
current_block = 9587810615301548814;
break ;
} else if unsafe{(*ctxt).progressive == 0 as libc::c_int &&
((*(*ctxt).input).end.offset_from((*(*ctxt).input).cur)
as libc::c_long) <
250 as libc::c_int as libc::c_long} {
xmlGROW(ctxt);
...
while j < nratts {
unsafe{
    if !(*atts.offset((i + 2 as libc::c_int) as
        isize)).is_null() {
        /*
...
*/
        let mut offset: ptrdiff_t =
            (*(*ctxt).input).base.offset_from(*atts.offset((i
                ...
                isize))
                as
                libc::c_long; /* Reset repurposed namespace URI */
        let ref mut fresh110 =
            *atts.offset((i + 2 as libc::c_int) as
                isize); /* value */
        *fresh110 = 0 as *const xmlChar;
        let ref mut fresh111 =
            *atts.offset((i + 3 as libc::c_int) as isize);
        *fresh111 = (*fresh111).offset(offset as isize);
        let ref mut fresh112 =
            *atts.offset((i + 4 as libc::c_int) as isize);
...
let mut defaults: xmlDefAttrsPtr = 0 as *mut xmlDefAttrs;
defaults =
    xmlHashLookup2_safe((safe_ctxt).attsDefault, localname, prefix)
        as xmlDefAttrsPtr;
if !defaults.is_null() {
    i = 0 as libc::c_int;
    loop  {
...
*/
            ...
            if !(j <= nbNs) {
                ...
                if unsafe{nsname != *(*defaults).values.as_mut_ptr().offset((5
                        as
                        libc::c_int
                        *
                        i
                        ...
                        isize)}
                {
                    if unsafe{nsPush(ctxt, 0 as *const xmlChar,
...
*/
            ...
            if !(j < nbatts) {
                if atts.is_null() ||
                    nbatts + 5 as libc::c_int > maxatts
                {
                ...
                unsafe{
                    ...
                    let ref mut fresh114 =
                        *atts.offset(fresh113 as isize);
                    *fresh114 = attname;
                    let fresh115 = nbatts;
                    nbatts = nbatts + 1;
                    let ref mut fresh116 =
                        *atts.offset(fresh115 as isize);
                    *fresh116 = aprefix;
                    if aprefix.is_null() {
                        let fresh117 = nbatts;
                        nbatts = nbatts + 1;
                        let ref mut fresh118 =
                            *atts.offset(fresh117 as isize);
                        *fresh118 = 0 as *const xmlChar
                    } else {
                        let fresh119 = nbatts;
                        nbatts = nbatts + 1;
                        let ref mut fresh120 =
                            *atts.offset(fresh119 as isize);
                        *fresh120 =
                ...
                if (safe_ctxt).standalone == 1 as libc::c_int
                    &&
                    unsafe{!(*(*defaults).values.as_mut_ptr().offset((5
                        as
                        libc::c_int
                        *
                        i
...
*/
        ...
        while j < i {
            unsafe{
                if *atts.offset(i as isize) ==
                    *atts.offset(j as isize) {
                    if *atts.offset((i + 1 as libc::c_int) as
                        isize) ==
                        *atts.offset((j + 1 as libc::c_int)
                            as isize) {
    ...
    if !prefix.is_null() && nsname.is_null() {
        xmlNsErr(ctxt, XML_NS_ERR_UNDEFINED_NAMESPACE,
                 b"Namespace prefix %s on %s is not defined\n\x00"
                     as *const u8 as *const libc::c_char,
                 prefix, localname, 0 as *const xmlChar);
    }
    unsafe{
        *pref = prefix;
        *URI = nsname;
...
* [NS 9] ETag ::= '</' QName S? '>'
*/
fn xmlParseEndTag2(mut ctxt: xmlParserCtxtPtr,
...
xmlSkipBlankChars(ctxt);
if unsafe{!(0x9 as libc::c_int <= *(*(*ctxt).input).cur as libc::c_int &&
*(*(*ctxt).input).cur as libc::c_int <= 0xa as libc::c_int ||
*(*(*ctxt).input).cur as libc::c_int == 0xd as libc::c_int ||
0x20 as libc::c_int <= *(*(*ctxt).input).cur as libc::c_int) ||
*(*(*ctxt).input).cur as libc::c_int != '>' as i32} {
...
xmlFatalErrMsgStrIntStr(ctxt, XML_ERR_TAG_NAME_MISMATCH,
            b"Opening and ending tag mismatch: %s line %d and %s\n\x00"
                as *const u8 as *const libc::c_char,
            (safe_ctxt).name, (safe_tag).line, name);
}
/*
* SAX: End of Tag
*/
unsafe{
if !(safe_ctxt).sax.is_null() && (*(*ctxt).sax).endElementNs.is_some() &&
(safe_ctxt).disableSAX == 0 {
(*(*ctxt).sax).endElementNs.expect("non-null function pointer")((safe_ctxt).userData,
                                                        (safe_ctxt).name,
                                                        (safe_tag).prefix,
                                                        (safe_tag).URI);
}
...
if if r < 0x100 as libc::c_int {
(0x9 as libc::c_int <= r && r <= 0xa as libc::c_int ||
r == 0xd as libc::c_int || 0x20 as libc::c_int <= r) as
libc::c_int
} else {
(0x100 as libc::c_int <= r && r <= 0xd7ff as libc::c_int ||
...
s == 0xd as libc::c_int || 0x20 as libc::c_int <= s) as
libc::c_int
} else {
...
xmlFatalErrMsgStr(ctxt, XML_ERR_CDATA_NOT_FINISHED,
      b"CData section not finished\n%.50s\n\x00" as
          *const u8 as *const libc::c_char, buf);
xmlFree_safe(buf as *mut libc::c_void);
return
}
unsafe{
...
libc::c_int == 'T' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(7 as libc::c_int as
isize) as
libc::c_int == 'A' as i32 &&
*((*(*ctxt).input).cur as
*mut libc::c_uchar).offset(8 as libc::c_int as
isize) as
libc::c_int == '[' as i32 {
xmlParseCDSect(ctxt);
} else if *cur as libc::c_int == '<' as i32 &&
...
*(*ctxt).nameTab.offset(((*ctxt).nameNr - 1 as libc::c_int) as
isize);
let mut line: libc::c_int =
(*(*ctxt).pushTab.offset(((*ctxt).nameNr - 1 as libc::c_int) as
isize)).line;
...
*/
fn xmlParseElementStart(mut ctxt: xmlParserCtxtPtr)
           -> libc::c_int {
let mut safe_ctxt  = unsafe{&mut *ctxt};
let mut name: *const xmlChar = 0 as *const xmlChar;
let mut prefix: *const xmlChar = 0 as *const xmlChar;
...
if (safe_ctxt).validate != 0 && (safe_ctxt).wellFormed != 0 &&
!(safe_ctxt).myDoc.is_null() && !(safe_ctxt).node.is_null() &&
...
pub fn xmlParseVersionNum(mut ctxt: xmlParserCtxtPtr)
             -> *mut xmlChar {
let mut buf: *mut xmlChar = 0 as *mut xmlChar;
let mut len: libc::c_int = 0 as libc::c_int;
let mut size: libc::c_int = 10 as libc::c_int;
let mut cur: xmlChar = 0;
buf = xmlMallocAtomic_safe((size as
libc::c_ulong).wrapping_mul(::std::mem::size_of::<xmlChar>()
as
libc::c_ulong))
as *mut xmlChar;
...
pub fn xmlParseVersionInfo(mut ctxt: xmlParserCtxtPtr)
              -> *mut xmlChar {
let mut version: *mut xmlChar = 0 as *mut xmlChar;
//@todo 削减unsafe范围
unsafe{
...
pub fn xmlParseEncName(mut ctxt: xmlParserCtxtPtr)
          -> *mut xmlChar {
let mut buf: *mut xmlChar = 0 as *mut xmlChar;
let mut len: libc::c_int = 0 as libc::c_int;
let mut size: libc::c_int = 10 as libc::c_int;
...
pub fn xmlParseEncodingDecl(mut ctxt: xmlParserCtxtPtr)
               -> *const xmlChar {
let mut safe_ctxt  = unsafe{&mut *ctxt};
...
*/
if unsafe{(*ctxt).encoding.is_null() && !(*(*ctxt).input).buf.is_null()
...
if !(safe_ctxt).encoding.is_null() {
xmlFree_safe((safe_ctxt).encoding
as
*mut xmlChar
as
*mut libc::c_void);
...
xmlFree_safe((*(*ctxt).input).encoding
    as
    *mut xmlChar
    as
    *mut libc::c_void);
...
handler =
xmlFindCharEncodingHandler_safe(encoding as *const libc::c_char);
if !handler.is_null() {
...
pub fn xmlParseSDDecl(mut ctxt: xmlParserCtxtPtr)
         -> libc::c_int {
...
xmlFatalErrMsgStr(ctxt, XML_ERR_UNKNOWN_VERSION,
              b"Unsupported version \'%s\'\n\x00" as
                  *const u8 as *const libc::c_char,
              version);
} else if unsafe{*version.offset(0 as libc::c_int as isize) as
libc::c_int == '1' as i32 &&
*version.offset(1 as libc::c_int as isize) as
libc::c_int == '.' as i32} {
...
xmlFree_safe((safe_ctxt).version as
*mut libc::c_void);
}
...
pub fn xmlParseDocument(mut ctxt: xmlParserCtxtPtr)
           -> libc::c_int {
let mut safe_ctxt  = unsafe{&mut *ctxt};
let mut start: [xmlChar; 4] = [0; 4];
let mut enc: xmlCharEncoding = XML_CHAR_ENCODING_NONE;
...
*(*(*ctxt).input).cur.offset(3 as libc::c_int as isize);
enc =
xmlDetectCharEncoding_safe(&mut *start.as_mut_ptr().offset(0 as
libc::c_int
as
isize),
                       4 as libc::c_int);
...
pub fn xmlParseExtParsedEnt(mut ctxt: xmlParserCtxtPtr)
               -> libc::c_int {
let mut safe_ctxt  = unsafe{&mut *ctxt};
let mut start: [xmlChar; 4] = [0; 4];
let mut enc: xmlCharEncoding = XML_CHAR_ENCODING_NONE;
...
*/
#[cfg(HAVE_parser_LIBXML_PUSH_ENABLED)]
fn xmlParseLookupSequence(mut ctxt: xmlParserCtxtPtr,
             mut first: xmlChar,
             mut next: xmlChar,
             mut third: xmlChar)
             -> libc::c_int {
...
if *buf.offset((base + 1 as libc::c_int) as isize) as
    ...
    *buf.offset((base + 2 as libc::c_int) as isize) as
        libc::c_int != third as libc::c_int {
...
return unsafe{(base as libc::c_long -
...
*/
#[cfg(HAVE_parser_LIBXML_PUSH_ENABLED)]
fn xmlParseGetLasts(mut ctxt: xmlParserCtxtPtr,
...
*/
#[cfg(HAVE_parser_LIBXML_PUSH_ENABLED)]
fn xmlCheckCdataPush(mut utf: *const xmlChar,
        mut len: libc::c_int,
...
if unsafe{*utf.offset((ix + 1 as libc::c_int) as isize) as libc::c_int &
0xc0 as libc::c_int != 0x80 as libc::c_int ||
*utf.offset((ix + 2 as libc::c_int) as isize) as
...
*utf.offset((ix + 2 as libc::c_int) as isize) as
    libc::c_int & 0xc0 as libc::c_int !=
    0x80 as libc::c_int ||
*utf.offset((ix + 3 as libc::c_int) as isize) as
    libc::c_int & 0xc0 as libc::c_int !=
    0x80 as libc::c_int {
return -ix
}
codepoint =
(*utf.offset(ix as isize) as libc::c_int & 0x7 as libc::c_int)
    << 18 as libc::c_int;
codepoint |=
...
*/
#[cfg(HAVE_parser_LIBXML_PUSH_ENABLED)]
fn xmlParseTryOrFinish(mut ctxt: xmlParserCtxtPtr,
...
if !(safe_ctxt).input.is_null() &&
unsafe{(*(*ctxt).input).cur.offset_from((*(*ctxt).input).base) as
libc::c_long > 4096 as libc::c_int as libc::c_long} {
xmlSHRINK(ctxt);
(safe_ctxt).checkIndex = 0 as libc::c_int as libc::c_long
}
...
if cur as libc::c_int == 0 as libc::c_int {
    ...
    unsafe{
        ...
        if *(*(*ctxt).input).cur.offset(2 as libc::c_int as
            isize) as
            libc::c_int == 'x' as i32 &&
            *(*(*ctxt).input).cur.offset(3 as libc::c_int
                as isize) as
                libc::c_int == 'm' as i32 &&
            *(*(*ctxt).input).cur.offset(4 as libc::c_int
                as isize) as
                libc::c_int == 'l' as i32 &&
            (*(*(*ctxt).input).cur.offset(5 as libc::c_int
                as isize) as
                libc::c_int == 0x20 as libc::c_int ||
                0x9 as libc::c_int <=
 ...
 */
            ...
            (*ctxt).version =
                xmlCharStrdup_safe(b"1.0\x00" as *const u8 as
                    *const libc::c_char);
            if !(*ctxt).sax.is_null() &&
                (*(*ctxt).sax).startDocument.is_some() &&
                (*ctxt).disableSAX == 0 {
    ...
    match () {
        #[cfg(HAVE_parser_LIBXML_VALID_ENABLED)] _ => {
            if (safe_ctxt).validate != 0 && (safe_ctxt).wellFormed != 0 &&
                ...
                (safe_ctxt).node == unsafe{(*(*ctxt).myDoc).children} {
                (safe_ctxt).valid &=
                    xmlValidateRoot_safe(&mut (safe_ctxt).vctxt,
                                    (safe_ctxt).myDoc)
            }
        }
        #[cfg(not(HAVE_parser_LIBXML_VALID_ENABLED))] _ => {}
    ...
    unsafe{
        if *(*(*ctxt).input).cur as libc::c_int == '/' as i32
            ...
            *(*(*ctxt).input).cur.offset(1 as libc::c_int
                ...
                libc::c_int == '>' as i32 {
            (*(*ctxt).input).cur =
                (*(*ctxt).input).cur.offset(2 as libc::c_int
                    as isize);
            (*(*ctxt).input).col += 2 as libc::c_int;
            if *(*(*ctxt).input).cur as libc::c_int ==
                0 as libc::c_int {
...
if cur as libc::c_int == '<' as i32 &&
    ...
    if terminate == 0 &&
        xmlParseLookupSequence(ctxt,
                               ...
                               0 as libc::c_int as
                                   xmlChar) <
            0 as libc::c_int {
        (safe_ctxt).progressive =
            XML_PARSER_PI as libc::c_int;
        current_block = 1672565932838553232;
        break ;
    } else {
        xmlParsePI(ctxt);
    ...
    next as libc::c_int == '!' as i32 &&
    unsafe{*(*(*ctxt).input).cur.offset(2 as
        libc::c_int
        as isize)
        as libc::c_int == '-' as i32 &&
    *(*(*ctxt).input).cur.offset(3 as
    ...
    unsafe{
        (*(*ctxt).input).cur =
            (*(*ctxt).input).cur.offset(4 as libc::c_int as
                isize);
    }
    term =
        xmlParseLookupSequence(ctxt,
    ...
    if terminate == 0 && term < 0 as libc::c_int {
        (safe_ctxt).progressive =
            XML_PARSER_COMMENT as libc::c_int;
        current_block = 1672565932838553232;
        break ;
    } else {
        xmlParseComment(ctxt);
    ...
    unsafe{*(*(*ctxt).input).cur.offset(1 as
        libc::c_int
        as isize)
        as libc::c_int == '!' as i32 &&
    *(*(*ctxt).input).cur.offset(2 as
    ...
    *(*(*ctxt).input).cur.offset(8 as
    ...
    unsafe{
        (*(*ctxt).input).cur =
            (*(*ctxt).input).cur.offset(9 as libc::c_int as
                isize);
        (*(*ctxt).input).col += 9 as libc::c_int;
        if *(*(*ctxt).input).cur as libc::c_int ==
            0 as libc::c_int {
...
unsafe{
    xmlParseEndTag2(ctxt,
                    &mut *(*ctxt).pushTab.offset(((*ctxt).nameNr
                        -
                        1 as
                            libc::c_int)
                        as
...
tmp = unsafe{xmlCheckCdataPush((*(*ctxt).input).cur,
                      300 as libc::c_int,
                      0 as libc::c_int)};
if tmp < 0 as libc::c_int {
    ...
    if unsafe{*(*(*ctxt).input).cur as libc::c_int ==
        0 as libc::c_int} {
        xmlParserInputGrow_safe((safe_ctxt).input,
                           250 as libc::c_int);
    }
    (safe_ctxt).checkIndex = 0 as libc::c_int as libc::c_long;
    current_block = 1672565932838553232;
...
tmp_0 =
    unsafe{xmlCheckCdataPush((*(*ctxt).input).cur, base_0,
                      1 as libc::c_int)};
...
if (*(*ctxt).input).buf.is_null() {
    avail =
        ((*(*ctxt).input).length as libc::c_long -
            (*(*ctxt).input).cur.offset_from((*(*ctxt).input).base)
                as libc::c_long) as libc::c_int
} else {
    avail =
...
if cur as libc::c_int == '<' as i32 &&
    ...
    *(*(*ctxt).input).cur.offset(7 as libc::c_int as
        isize) as
        libc::c_int == 'P' as i32 &&
    *(*(*ctxt).input).cur.offset(8 as libc::c_int as
        isize) as
        libc::c_int == 'E' as i32 {
    if terminate == 0 &&
        xmlParseLookupSequence(ctxt, '>' as i32 as xmlChar,
...
*/
            ...
            if !(safe_ctxt).sax.is_null() &&
                ...
                (*(*ctxt).sax).externalSubset.expect("non-null function pointer")((safe_ctxt).userData,
...
next as libc::c_int == '!' as i32 &&
unsafe{*(*(*ctxt).input).cur.offset(2 as libc::c_int as
    isize) as
    libc::c_int == '-' as i32 &&
*(*(*ctxt).input).cur.offset(3 as libc::c_int as
    isize) as
...
if (safe_ctxt).progressive == 0 as libc::c_int {
    (safe_ctxt).progressive =
        XML_PARSER_START_TAG as libc::c_int
}
xmlParseGetLasts(ctxt, &mut lastlt, &mut lastgt);
...
if quote as libc::c_int != 0 as libc::c_int {
    ...
    if quote as libc::c_int == 0 as libc::c_int &&
        unsafe{*buf.offset(base_1 as isize) as libc::c_int
            == '<' as i32} {
        let mut found: libc::c_int = 0 as libc::c_int;
        ...
        if unsafe{((base_1 as
            ...
            *buf.offset((base_1 + 1 as libc::c_int)
                as isize) as
                libc::c_int == '!' as i32 &&
            *buf.offset((base_1 + 2 as libc::c_int)
                as isize) as
                libc::c_int == '-' as i32 &&
            *buf.offset((base_1 + 3 as libc::c_int)
                as isize) as
                libc::c_int == '-' as i32} {
            while ((base_1 as
                ...
                libc::c_uint)
                as libc::c_ulong) < use_0 {
                if unsafe{*buf.offset(base_1 as isize) as
                    libc::c_int == '-' as i32 &&
                    *buf.offset((base_1 +
                        1 as
                            libc::c_int)
...
unsafe {
snprintf(buffer.as_mut_ptr(), 149 as libc::c_int as libc::c_ulong,
