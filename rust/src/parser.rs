unsafe fn xmlParserEntityCheck(
        ...
        && (*ctxt).errNo != XML_ERR_ENTITY_LOOP as libc::c_int
    {
        ...
        (*ent).checked = diff.wrapping_mul(2 as libc::c_int as libc::c_ulong) as libc::c_int;
        if !rep.is_null() {
            ...
            == 0 as libc::c_int as libc::c_ulong
    {
        ...
        while i < (*ctxt).inputNr {
            consumed = (consumed as libc::c_ulong).wrapping_add(
    ...
    if replacement != 0 as libc::c_int as libc::c_ulong {
        ...
        consumed =
            (consumed as libc::c_ulong).wrapping_add((*ctxt).sizeentities) as size_t as size_t;
...
unsafe fn xmlValidityError(
    ...
    if !ctxt.is_null() {
        (*ctxt).errNo = error as libc::c_int;
        if !(*ctxt).sax.is_null() && (*(*ctxt).sax).initialized == 0xdeedbeaf as libc::c_uint {
            schannel = (*(*ctxt).sax).serror
        }
    }
    if !ctxt.is_null() {
...
pub unsafe fn xmlHasFeature(mut feature: xmlFeature) -> libc::c_int {
...
unsafe fn xmlDetectSAX2(mut ctxt: xmlParserCtxtPtr) {
    ...
    match () {
        ...
        _ => {
            if !sax.is_null()
                && (*sax).initialized == 0xdeedbeaf as libc::c_uint
        ...
        13183875560443969876 => {
            ...
            if defaults.is_null() {
                ...
                if defaults.is_null() {
                    ...
                    {
                        xmlFree.expect("non-null function pointer")(defaults as *mut libc::c_void);
                        current_block = 2968889880470072775;
                    } else {
            ...
            match current_block {
                ...
                _ => {
                    ...
                    if name.is_null() {
                        name = xmlDictLookup((*ctxt).dict, fullattr, -(1 as libc::c_int));
                        prefix = 0 as *const xmlChar
                    } else {
                        name = xmlDictLookup((*ctxt).dict, name, -(1 as libc::c_int));
                    ...
                    *fresh6 = value.offset(len as isize);
                    if (*ctxt).external != 0 {
                        let ref mut fresh7 = *(*defaults).values.as_mut_ptr().offset(
                            (5 as libc::c_int * (*defaults).nbAttrs + 4 as libc::c_int) as isize,
                        );
                        *fresh7 =
...
pub unsafe fn xmlCheckLanguageID(mut lang: *const xmlChar) -> libc::c_int {
    let mut current_block: u64;
            ...
            && *cur.offset(1 as libc::c_int as isize) as libc::c_int == '-' as i32
    {
        ...
        return (*cur.offset(0 as libc::c_int as isize) as libc::c_int == 0 as libc::c_int)
            as libc::c_int;
    }
    nxt = cur;
        ...
        && *nxt.offset(0 as libc::c_int as isize) as libc::c_int <= '9' as i32
    {
        ...
        } else if nxt.offset_from(cur) as libc::c_long == 2 as libc::c_int as libc::c_long {
            current_block = 15970415187932728765;
        } else if nxt.offset_from(cur) as libc::c_long >= 5 as libc::c_int as libc::c_long
 ...
 */
#[cfg(HAVE_parser_SAX2)]
unsafe fn nsPop(mut ctxt: xmlParserCtxtPtr, mut nr: libc::c_int) -> libc::c_int {
...
unsafe fn xmlCtxtGrowAttrs(mut ctxt: xmlParserCtxtPtr, mut nr: libc::c_int) -> libc::c_int {
    let mut current_block: u64;
...
pub unsafe fn inputPush_parser(
    mut ctxt: xmlParserCtxtPtr,
...
pub unsafe fn nodePush(mut ctxt: xmlParserCtxtPtr, mut value: xmlNodePtr) -> libc::c_int {
    if ctxt.is_null() {
        return 0 as libc::c_int;
    }
...
unsafe fn nameNsPush(
    ...
    match current_block {
        ...
        _ => {
            ...
            *fresh20 = value;
            (*ctxt).name = value;
            tag = &mut *(*ctxt).pushTab.offset((*ctxt).nameNr as isize) as *mut xmlStartTag;
            (*tag).prefix = prefix;
            (*tag).URI = URI;
            (*tag).line = line;
            (*tag).nsNr = nsNr;
            let fresh21 = (*ctxt).nameNr;
...
pub unsafe fn namePush(mut ctxt: xmlParserCtxtPtr, mut value: *const xmlChar) -> libc::c_int {
    ...
    let ref mut fresh23 = *(*ctxt).nameTab.offset((*ctxt).nameNr as isize);
    *fresh23 = value;
    (*ctxt).name = value;
...
unsafe fn spacePush(mut ctxt: xmlParserCtxtPtr, mut val: libc::c_int) -> libc::c_int {
    ...
    *(*ctxt).spaceTab.offset((*ctxt).spaceNr as isize) = val;
    (*ctxt).space = &mut *(*ctxt).spaceTab.offset((*ctxt).spaceNr as isize) as *mut libc::c_int;
    let fresh26 = (*ctxt).spaceNr;
...
unsafe fn spacePop(mut ctxt: xmlParserCtxtPtr) -> libc::c_int {
    ...
    if (*ctxt).spaceNr > 0 as libc::c_int {
        (*ctxt).space = &mut *(*ctxt)
            .spaceTab
            .offset(((*ctxt).spaceNr - 1 as libc::c_int) as isize)
            as *mut libc::c_int
    } else {
...
unsafe fn xmlGROW(mut ctxt: xmlParserCtxtPtr) {
    ...
    if !(*(*ctxt).input).cur.is_null() && *(*(*ctxt).input).cur as libc::c_int == 0 as libc::c_int {
        xmlParserInputGrow((*ctxt).input, 250 as libc::c_int);
    };
}
/* *
...
pub unsafe fn xmlParseCharRef(mut ctxt: xmlParserCtxtPtr) -> libc::c_int {
        ...
        && *(*(*ctxt).input).cur.offset(2 as libc::c_int as isize) as libc::c_int == 'x' as i32
    {
        ...
        while *(*(*ctxt).input).cur as libc::c_int != ';' as i32 {
            ...
            {
                val = val * 16 as libc::c_int + (*(*(*ctxt).input).cur as libc::c_int - '0' as i32)
        ...
        && *(*(*ctxt).input).cur.offset(1 as libc::c_int as isize) as libc::c_int == '#' as i32
    {
        ...
        while *(*(*ctxt).input).cur as libc::c_int != ';' as i32 {
            ...
            if *(*(*ctxt).input).cur as libc::c_int >= '0' as i32
                && *(*(*ctxt).input).cur as libc::c_int <= '9' as i32
            {
                val = val * 10 as libc::c_int + (*(*(*ctxt).input).cur as libc::c_int - '0' as i32);
                if val > 0x110000 as libc::c_int {
                    val = 0x110000 as libc::c_int
                }
                xmlNextChar(ctxt);
        ...
        's_81: loop {
                ...
                && *str.offset(1 as libc::c_int as isize) as libc::c_int == '#' as i32
            {
                ...
                if 0 as libc::c_int == 1 as libc::c_int {
                    ...
                    *buffer.offset(fresh29 as isize) = val as xmlChar
                } else {
                    nbchars = (nbchars as libc::c_ulong).wrapping_add(xmlCopyCharMultiByte(
                    ...
                    if !(*ent).content.is_null() {
                        if 0 as libc::c_int == 1 as libc::c_int {
                            ...
                            *buffer.offset(fresh30 as isize) =
                            ...
                            nbchars = (nbchars as libc::c_ulong).wrapping_add(xmlCopyCharMultiByte(
                                &mut *buffer.offset(nbchars as isize),
                                *(*ent).content.offset(0 as libc::c_int as isize) as libc::c_int,
                            )
                    ...
                    if rep.is_null() {
                        *(*ent).content.offset(0 as libc::c_int as isize) =
                        ...
                        while *current as libc::c_int != 0 as libc::c_int {
                            ...
                            let fresh32 = nbchars;
                            nbchars = nbchars.wrapping_add(1);
                            *buffer.offset(fresh32 as isize) = *fresh31;
                ...
                if !ent.is_null() {
                    if (*ent).content.is_null() {
                         ...
                         * internal subset
                         */
                        if (*ctxt).options & XML_PARSE_NOENT as libc::c_int != 0 as libc::c_int
                ...
                if l == 1 as libc::c_int {
                    let fresh39 = nbchars;
                    nbchars = nbchars.wrapping_add(1);
                    *buffer.offset(fresh39 as isize) = c as xmlChar
                } else {
                    nbchars = (nbchars as libc::c_ulong).wrapping_add(xmlCopyCharMultiByte(
                        &mut *buffer.offset(nbchars as isize),
                        c,
...
unsafe fn areBlanks(
    ...
    if lastChild.is_null() {
        ...
        if !(*(*ctxt).node).children.is_null() && xmlNodeIsText((*(*ctxt).node).children) != 0 {
            return 0 as libc::c_int;
        }
    }
    return 1 as libc::c_int;
...
pub unsafe fn xmlSplitQName(
    ...
    *prefix = 0 as *mut xmlChar;
    ...
    match () {
        ...
        _ => {
            if *cur.offset(0 as libc::c_int as isize) as libc::c_int == 'x' as i32 {
                if *cur.offset(1 as libc::c_int as isize) as libc::c_int == 'm' as i32 {
                    if *cur.offset(2 as libc::c_int as isize) as libc::c_int == 'l' as i32 {
                        if *cur.offset(3 as libc::c_int as isize) as libc::c_int == ':' as i32 {
...
unsafe fn xmlParseNameComplex(mut ctxt: xmlParserCtxtPtr) -> *const xmlChar {
    ...
    if *(*(*ctxt).input).cur as libc::c_int == '\n' as i32
        && *(*(*ctxt).input).cur.offset(-(1 as libc::c_int) as isize) as libc::c_int == '\r' as i32
    {
        ...
        if len >= 100 as libc::c_int {
            ...
            while xmlIsNameChar(ctxt, c) != 0 {
                ...
                if l == 1 as libc::c_int {
                    let fresh58 = len;
                    len = len + 1;
                    *buffer.offset(fresh58 as isize) = c as xmlChar
                } else {
                    len += xmlCopyCharMultiByte(&mut *buffer.offset(len as isize), c)
                }
                if *(*(*ctxt).input).cur as libc::c_int == '\n' as i32 {
    ...
    if !((*ctxt).instate as libc::c_int == XML_PARSER_EOF as libc::c_int) {
         ...
         */
        {
            if !((if c < 0x100 as libc::c_int {
                ...
                && (c != stop as libc::c_int || (*ctxt).input != input)
                && (*ctxt).instate as libc::c_int != XML_PARSER_EOF as libc::c_int)
            {
            ...
            if l == 1 as libc::c_int {
                let fresh59 = len;
                len = len + 1;
                *buf.offset(fresh59 as isize) = c as xmlChar
            } else {
                ...
                *buf.offset(len as isize) = 0 as libc::c_int as xmlChar;
                if !((*ctxt).instate as libc::c_int == XML_PARSER_EOF as libc::c_int) {
                    if c != stop as libc::c_int {
                        xmlFatalErr(ctxt, XML_ERR_ENTITY_NOT_FINISHED, 0 as *const libc::c_char);
                    } else {
                        xmlNextChar(ctxt);
                        /*
                        ...
                        loop {
                                        ...
                                        != '#' as i32
                            {
                                ...
                                if nameOk == 0 as libc::c_int || *cur as libc::c_int != ';' as i32 {
...
unsafe fn xmlParseAttValueComplex(
    ...
    let mut current: *mut xmlChar = 0 as *mut xmlChar;
    let mut ent: xmlEntityPtr = 0 as *mut xmlEntity;
    if *(*(*ctxt).input).cur.offset(0 as libc::c_int as isize) as libc::c_int == '\"' as i32 {
        (*ctxt).instate = XML_PARSER_ATTRIBUTE_VALUE;
        limit = '\"' as i32 as xmlChar;
        xmlNextChar(ctxt);
    } else if *(*(*ctxt).input).cur.offset(0 as libc::c_int as isize) as libc::c_int == '\'' as i32
    {
        limit = '\'' as i32 as xmlChar;
        (*ctxt).instate = XML_PARSER_ATTRIBUTE_VALUE;
        xmlNextChar(ctxt);
                ...
                if c == '&' as i32 {
                        ...
                        == '#' as i32
                    {
                        ...
                        if val == '&' as i32 {
                            ...
                            len = (len as libc::c_ulong).wrapping_add(xmlCopyChar(
                                0 as libc::c_int,
                        ...
                        } else if !ent.is_null() && (*ctxt).replaceEntities != 0 as libc::c_int {
                        ...
                        || c == 0x9 as libc::c_int
                    {
                        if len != 0 as libc::c_int as libc::c_ulong || normalize == 0 {
                            if normalize == 0 || in_space == 0 {
                                if l == 1 as libc::c_int {
                                    let fresh80 = len;
                                    len = len.wrapping_add(1);
                                    *buf.offset(fresh80 as isize) = 0x20 as libc::c_int as xmlChar
                                } else {
                                    len = (len as libc::c_ulong).wrapping_add(xmlCopyCharMultiByte(
                                        &mut *buf.offset(len as isize),
                                        0x20 as libc::c_int,
                                    )
                                        as libc::c_ulong)
                        ...
                        if l == 1 as libc::c_int {
                            let fresh81 = len;
                            len = len.wrapping_add(1);
                            *buf.offset(fresh81 as isize) = c as xmlChar
                        } else {
                            len = (len as libc::c_ulong).wrapping_add(xmlCopyCharMultiByte(
                                &mut *buf.offset(len as isize),
                                c,
            ...
            _ => {
                if (*ctxt).instate as libc::c_int == XML_PARSER_EOF as libc::c_int {
                    ...
                    *buf.offset(len as isize) = 0 as libc::c_int as xmlChar;
                    ...
                    } else if *(*(*ctxt).input).cur as libc::c_int != limit as libc::c_int {
                        if c != 0 as libc::c_int
                            && (if c < 0x100 as libc::c_int {
                                (0x9 as libc::c_int <= c && c <= 0xa as libc::c_int
                                    || c == 0xd as libc::c_int
...
pub unsafe fn xmlParseSystemLiteral(mut ctxt: xmlParserCtxtPtr) -> *mut xmlChar {
        ...
        && cur != stop as libc::c_int
    {
        ...
        if l == 1 as libc::c_int {
            let fresh82 = len;
            len = len + 1;
            *buf.offset(fresh82 as isize) = cur as xmlChar
        } else {
        ...
        loop {
            ...
            if *in_0 as libc::c_int == 0xa as libc::c_int {
                ...
                if *in_0 as libc::c_int == '<' as i32 {
                    nbchar = in_0.offset_from((*(*ctxt).input).cur) as libc::c_long as libc::c_int;
...
unsafe fn xmlParseCharDataComplex(mut ctxt: xmlParserCtxtPtr, mut cdata: libc::c_int) {
        ...
        }) != 0
    {
        ...
        if l == 1 as libc::c_int {
            ...
            nbchar += xmlCopyCharMultiByte(&mut *buf.as_mut_ptr().offset(nbchar as isize), cur)
        }
...
pub unsafe fn xmlParseExternalID(
    ...
    *publicID = 0 as *mut xmlChar;
    ...
    } else if *((*(*ctxt).input).cur as *mut libc::c_uchar).offset(0 as libc::c_int as isize)
        as libc::c_int
        == 'P' as i32
        && *((*(*ctxt).input).cur as *mut libc::c_uchar).offset(1 as libc::c_int as isize)
            as libc::c_int
            ...
            if !(cur == 0 as libc::c_int) {
                    ...
                    && (cur != '>' as i32 || r != '-' as i32 || q != '-' as i32)
                {
                    ...
                    if ql == 1 as libc::c_int {
                        let fresh85 = len;
                        len = len.wrapping_add(1);
                        *buf.offset(fresh85 as isize) = q as xmlChar
                    } else {
                        len = (len as libc::c_ulong).wrapping_add(xmlCopyCharMultiByte(
                            &mut *buf.offset(len as isize),
                            q,
...
pub unsafe fn xmlParseComment(mut ctxt: xmlParserCtxtPtr) {
    ...
    loop {
        ...
        loop {
            ccol = (*(*ctxt).input).col;
            while *in_0 as libc::c_int > '-' as i32 && *in_0 as libc::c_int <= 0x7f as libc::c_int
                || *in_0 as libc::c_int >= 0x20 as libc::c_int
                    && (*in_0 as libc::c_int) < '-' as i32
                || *in_0 as libc::c_int == 0x9 as libc::c_int
            {
...
unsafe fn xmlParseCatalogPI(mut ctxt: xmlParserCtxtPtr, mut catalog: *const xmlChar) {
    ...
    while *tmp as libc::c_int == 0x20 as libc::c_int
        || 0x9 as libc::c_int <= *tmp as libc::c_int && *tmp as libc::c_int <= 0xa as libc::c_int
    ...
    ) != 0)
    {
        ...
        if !(marker as libc::c_int != '\'' as i32 && marker as libc::c_int != '\"' as i32) {
            ...
            if !(*tmp as libc::c_int == 0 as libc::c_int) {
                URL = xmlStrndup(base, tmp.offset_from(base) as libc::c_long as libc::c_int);
                tmp = tmp.offset(1);
                while *tmp as libc::c_int == 0x20 as libc::c_int
                    || 0x9 as libc::c_int <= *tmp as libc::c_int
                        && *tmp as libc::c_int <= 0xa as libc::c_int
        ...
        if !target.is_null() {
                        ...
                        != '>' as i32)
            {
                ...
                if l == 1 as libc::c_int {
                    let fresh86 = len;
                    len = len.wrapping_add(1);
                    *buf.offset(fresh86 as isize) = cur as xmlChar
                } else {
                    len = (len as libc::c_ulong).wrapping_add(xmlCopyCharMultiByte(
                        &mut *buf.offset(len as isize),
                        cur,
...
unsafe fn xmlParseElementChildrenContentDeclPriv(
        ...
        && (*ctxt).instate as libc::c_int != XML_PARSER_EOF as libc::c_int
    {
        ...
        if *(*(*ctxt).input).cur as libc::c_int == ',' as i32 {
            ...
            xmlNextChar(ctxt);
            op =
                xmlNewDocElementContent((*ctxt).myDoc, 0 as *const xmlChar, XML_ELEMENT_CONTENT_OR);
            if op.is_null() {
                if !last.is_null() && last != ret {
                    xmlFreeDocElementContent((*ctxt).myDoc, last);
                }
    ...
    if *(*(*ctxt).input).cur as libc::c_int == '?' as i32 {
        ...
        if !ret.is_null() {
            ...
            if (*ret).ocur as libc::c_uint == XML_ELEMENT_CONTENT_OPT as libc::c_int as libc::c_uint
                || (*ret).ocur as libc::c_uint
                    == XML_ELEMENT_CONTENT_MULT as libc::c_int as libc::c_uint
            {
                (*ret).ocur = XML_ELEMENT_CONTENT_MULT
...
pub unsafe fn xmlParseElementDecl(mut ctxt: xmlParserCtxtPtr) -> libc::c_int {
            ...
            == 'T' as i32
    {
                ...
                == 'Y' as i32
        {
            ...
            && *(*(*ctxt).input).cur.offset(1 as libc::c_int as isize) as libc::c_int == 'N' as i32
            && *(*(*ctxt).input).cur.offset(2 as libc::c_int as isize) as libc::c_int == 'Y' as i32
        {
            (*(*ctxt).input).cur = (*(*ctxt).input).cur.offset(3 as libc::c_int as isize);
            (*(*ctxt).input).col += 3 as libc::c_int;
            if *(*(*ctxt).input).cur as libc::c_int == 0 as libc::c_int {
        ...
        if *(*(*ctxt).input).cur.offset(1 as libc::c_int as isize) as libc::c_int == '!' as i32 {
            match *(*(*ctxt).input).cur.offset(2 as libc::c_int as isize) as libc::c_int {
                69 => {
                    ...
                    } else if *(*(*ctxt).input).cur.offset(3 as libc::c_int as isize) as libc::c_int
                        == 'N' as i32
                    {
...
pub unsafe fn xmlParseTextDecl(mut ctxt: xmlParserCtxtPtr) {
    ...
    if version.is_null() {
        version = xmlCharStrdup(b"1.0\x00" as *const u8 as *const libc::c_char)
...
pub unsafe fn xmlParseExternalSubset(
    ...
    if (*ctxt).myDoc.is_null() {
        (*ctxt).myDoc = xmlNewDoc(b"1.0\x00" as *const u8 as *const libc::c_char as *mut xmlChar);
        if (*ctxt).myDoc.is_null() {
            xmlErrMemory(
                ctxt,
                b"New Doc failed\x00" as *const u8 as *const libc::c_char,
...
pub unsafe fn xmlParseReference(mut ctxt: xmlParserCtxtPtr) {
    ...
    if *(*(*ctxt).input).cur.offset(1 as libc::c_int as isize) as libc::c_int == '#' as i32 {
        ...
        if (*ctxt).charset != XML_CHAR_ENCODING_UTF8 as libc::c_int {
            ...
            if 0 as libc::c_int == 1 as libc::c_int {
                ...
                i += xmlCopyCharMultiByte(&mut *out.as_mut_ptr().offset(i as isize), value)
            }
    ...
    if ((*ent).checked == 0 as libc::c_int
        || (*ent).children.is_null() && (*ctxt).options & XML_PARSE_NOENT as libc::c_int != 0)
        && ((*ent).etype as libc::c_uint
                ...
                != 0)
    {
        ...
        {
            (*ctxt).depth += 1;
            ret = xmlParseBalancedChunkMemoryInternal(ctxt, (*ent).content, user_data, &mut list);
            (*ctxt).depth -= 1
    ...
    if (*ctxt).replaceEntities != 0 || (*ent).children.is_null() {
        ...
        if !(*ctxt).node.is_null() && !(*ent).children.is_null() {
                    ...
                    == XML_PARSE_READER as libc::c_int as libc::c_uint
            {
                ...
                if xmlParserEntityCheck(ctxt, 0 as libc::c_int as size_t, ent, (*ctxt).sizeentcopy)
                    != 0
                {
                    return;
                }
...
pub unsafe fn xmlParseEntityRef(mut ctxt: xmlParserCtxtPtr) -> xmlEntityPtr {
    ...
    if !(*ctxt).sax.is_null() {
        if (*(*ctxt).sax).getEntity.is_some() {
            ent =
                (*(*ctxt).sax).getEntity.expect("non-null function pointer")((*ctxt).userData, name)
        }
        if (*ctxt).wellFormed == 1 as libc::c_int
            && ent.is_null()
...
unsafe fn xmlParseInternalSubset(mut ctxt: xmlParserCtxtPtr) {
    ...
    if *(*(*ctxt).input).cur as libc::c_int == '[' as i32 {
         ...
         */
        while (*(*(*ctxt).input).cur as libc::c_int != ']' as i32 || (*ctxt).inputNr > baseInputNr)
            && (*ctxt).instate as libc::c_int != XML_PARSER_EOF as libc::c_int
        {
            let mut check: *const xmlChar = (*(*ctxt).input).cur;
            let mut cons: libc::c_uint = (*(*ctxt).input).consumed as libc::c_uint;
...
unsafe fn xmlParseAttValueInternal(
    ...
    if normalize != 0 {
         ...
         */
        while in_0 < end
            && *in_0 as libc::c_int != limit as libc::c_int
            && (*in_0 as libc::c_int == 0x20 as libc::c_int
                || *in_0 as libc::c_int == 0x9 as libc::c_int
                || *in_0 as libc::c_int == 0xa as libc::c_int
                || *in_0 as libc::c_int == 0xd as libc::c_int)
            ...
            && *in_0 as libc::c_int != '<' as i32
        {
            ...
            let fresh96 = in_0;
            in_0 = in_0.offset(1);
            if *fresh96 as libc::c_int == 0x20 as libc::c_int
                && *in_0 as libc::c_int == 0x20 as libc::c_int
            {
                break;
            }
    ...
    *value = 0 as *mut xmlChar;
    ...
    if *prefix == (*ctxt).str_xml {
        ...
        if !internal_val.is_null() {
            xmlFree.expect("non-null function pointer")(internal_val as *mut libc::c_void);
        }
    }
    *value = val;
    return name;
        ...
        if !(attname.is_null() || attvalue.is_null()) {
            ...
            if attname == (*ctxt).str_xmlns && aprefix.is_null() {
                ...
                if URL.is_null() {
                    xmlErrMemory(
                        ctxt,
                        b"dictionary allocation failure\x00" as *const u8 as *const libc::c_char,
                    );
...
pub unsafe fn xmlParseContent(mut ctxt: xmlParserCtxtPtr) {
    ...
    if (*ctxt).instate as libc::c_int != XML_PARSER_EOF as libc::c_int && (*ctxt).nameNr > nameNr {
...
pub unsafe fn xmlParseVersionNum(mut ctxt: xmlParserCtxtPtr) -> *mut xmlChar {
    ...
    let fresh127 = len;
    len = len + 1;
    *buf.offset(fresh127 as isize) = cur;
    xmlNextChar(ctxt);
    cur = *(*(*ctxt).input).cur;
    if cur as libc::c_int != '.' as i32 {
        xmlFree.expect("non-null function pointer")(buf as *mut libc::c_void);
    ...
    let fresh128 = len;
    len = len + 1;
    *buf.offset(fresh128 as isize) = cur;
    xmlNextChar(ctxt);
    cur = *(*(*ctxt).input).cur;
...
pub unsafe fn xmlParseXMLDecl(mut ctxt: xmlParserCtxtPtr) {
    ...
    if version.is_null() {
        ...
        ) == 0
        {
            ...
            } else if *version.offset(0 as libc::c_int as isize) as libc::c_int == '1' as i32
        ...
        if !(*ctxt).version.is_null() {
            xmlFree.expect("non-null function pointer")((*ctxt).version as *mut libc::c_void);
        }
...
pub unsafe fn xmlParseDocument(mut ctxt: xmlParserCtxtPtr) -> libc::c_int {
                ...
                == 0xd as libc::c_int)
    {
        ...
        (*ctxt).version = xmlCharStrdup(b"1.0\x00" as *const u8 as *const libc::c_char)
    }
...
unsafe fn xmlCheckCdataPush(
    ...
    while ix < len {
        ...
        if c as libc::c_int & 0x80 as libc::c_int == 0 as libc::c_int {
            ...
            if *utf.offset((ix + 1 as libc::c_int) as isize) as libc::c_int & 0xc0 as libc::c_int
                != 0x80 as libc::c_int
            {
                return -ix;
            }
            codepoint =
                (*utf.offset(ix as isize) as libc::c_int & 0x1f as libc::c_int) << 6 as libc::c_int;
            codepoint |=
                *utf.offset((ix + 1 as libc::c_int) as isize) as libc::c_int & 0x3f as libc::c_int;
            ...
            codepoint =
                (*utf.offset(ix as isize) as libc::c_int & 0xf as libc::c_int) << 12 as libc::c_int;
            codepoint |= (*utf.offset((ix + 1 as libc::c_int) as isize) as libc::c_int
                & 0x3f as libc::c_int)
                << 6 as libc::c_int;
            codepoint |=
                *utf.offset((ix + 2 as libc::c_int) as isize) as libc::c_int & 0x3f as libc::c_int;
            if if codepoint < 0x100 as libc::c_int {
                (0x9 as libc::c_int <= codepoint && codepoint <= 0xa as libc::c_int
                    || codepoint == 0xd as libc::c_int
                    || 0x20 as libc::c_int <= codepoint) as libc::c_int
            ...
            codepoint =
                (*utf.offset(ix as isize) as libc::c_int & 0x7 as libc::c_int) << 18 as libc::c_int;
            codepoint |= (*utf.offset((ix + 1 as libc::c_int) as isize) as libc::c_int
                & 0x3f as libc::c_int)
                << 12 as libc::c_int;
            codepoint |= (*utf.offset((ix + 2 as libc::c_int) as isize) as libc::c_int
                ...
                << 6 as libc::c_int;
            codepoint |=
                *utf.offset((ix + 3 as libc::c_int) as isize) as libc::c_int & 0x3f as libc::c_int;
            if if codepoint < 0x100 as libc::c_int {
                (0x9 as libc::c_int <= codepoint && codepoint <= 0xa as libc::c_int
                    || codepoint == 0xd as libc::c_int
                    || 0x20 as libc::c_int <= codepoint) as libc::c_int
...
unsafe fn xmlParseTryOrFinish(
    ...
    match () {
        #[cfg(HAVE_parser_DEBUG_PUSH)]
        _ => match (*ctxt).instate as libc::c_int {
            -1 => {
                (*__xmlGenericError()).expect("non-null function pointer")(
                    *__xmlGenericErrorContext(),
                    b"PP: try EOF\n\x00" as *const u8 as *const libc::c_char,
    ...
    loop {
        ...
        if (*ctxt).errNo != XML_ERR_OK as libc::c_int && (*ctxt).disableSAX == 1 as libc::c_int {
            return 0 as libc::c_int;
        }
        if (*ctxt).input.is_null() {
            current_block = 1672565932838553232;
        ...
        match (*ctxt).instate as libc::c_int {
            ...
            0 => {
                if (*ctxt).charset == XML_CHAR_ENCODING_NONE as libc::c_int {
                    ...
                    } else if cur as libc::c_int == '<' as i32 && next as libc::c_int == '?' as i32
                    {
                        ...
                        if *(*(*ctxt).input).cur.offset(2 as libc::c_int as isize) as libc::c_int
                            == 'x' as i32
                            && *(*(*ctxt).input).cur.offset(3 as libc::c_int as isize)
                                as libc::c_int
                                == 'm' as i32
                                    ...
                                    == 0xd as libc::c_int)
                        {
                            ...
                            (*ctxt).version =
                                xmlCharStrdup(b"1.0\x00" as *const u8 as *const libc::c_char);
                            if !(*ctxt).sax.is_null()
                                && (*(*ctxt).sax).startDocument.is_some()
                                && (*ctxt).disableSAX == 0
                            {
                ...
                if cur as libc::c_int == '<' as i32 && next as libc::c_int == '/' as i32 {
                            ...
                            == '[' as i32
                    {
                        ...
                        if cur as libc::c_int == '&' as i32 {
                             ...
                             *    of the parser.
                             */
                            if (*ctxt).inputNr == 1 as libc::c_int && avail < 300 as libc::c_int {
                                if terminate == 0 {
                                    if (*ctxt).progressive != 0 {
            ...
            8 => {
                ...
                if base_0 < 0 as libc::c_int {
                    ...
                    tmp_0 = xmlCheckCdataPush((*(*ctxt).input).cur, base_0, 1 as libc::c_int);
            ...
            3 => {
                ...
                's_1946: loop {
                    ...
                    if quote as libc::c_int != 0 as libc::c_int {
                        if *buf.offset(base_1 as isize) as libc::c_int == quote as libc::c_int {
                            ...
                            && *buf.offset(base_1 as isize) as libc::c_int == '<' as i32
                        {
                            ...
                            if ((base_1 as libc::c_uint)
                                ...
                                as libc::c_ulong)
                                < use_0
                                && *buf.offset((base_1 + 1 as libc::c_int) as isize) as libc::c_int
                                    == '!' as i32
                                && *buf.offset((base_1 + 2 as libc::c_int) as isize) as libc::c_int
                                    == '-' as i32
                                && *buf.offset((base_1 + 3 as libc::c_int) as isize) as libc::c_int
                                    == '-' as i32
                            {
            ...
            15 => {
                (*__xmlGenericError()).expect("non-null function pointer")(
                    *__xmlGenericErrorContext(),
                    b"PP: internal error, state == IGNORE\x00" as *const u8 as *const libc::c_char,
                );
                (*ctxt).instate = XML_PARSER_DTD;
...
pub unsafe fn xmlParseChunk(
            ...
            && (*ctxt).instate as libc::c_int != XML_PARSER_EOF as libc::c_int
        {
                ...
                && !(*(*(*ctxt).input).buf).encoder.is_null()
            {
                ...
                if (*(*(*ctxt).input).buf).rawconsumed < len as libc::c_ulong {
                    len = (len as libc::c_ulong).wrapping_sub((*(*(*ctxt).input).buf).rawconsumed)
    ...
    if end_in_lf == 1 as libc::c_int && !(*ctxt).input.is_null() && !(*(*ctxt).input).buf.is_null()
    {
...
pub unsafe fn xmlCreatePushParserCtxt(
    ...
    if !sax.is_null() {
        ...
        (*ctxt).sax = xmlMalloc.expect("non-null function pointer")(::std::mem::size_of::<
            xmlSAXHandler,
        >() as libc::c_ulong) as xmlSAXHandlerPtr;
        if (*ctxt).sax.is_null() {
            xmlErrMemory(ctxt, 0 as *const libc::c_char);
            xmlFreeParserInputBuffer(buf);
...
unsafe fn xmlHaltParser(mut ctxt: xmlParserCtxtPtr) {
    ...
    if !(*ctxt).input.is_null() {
        ...
        if !(*(*ctxt).input).buf.is_null() {
            ...
            (*(*ctxt).input).buf = 0 as xmlParserInputBufferPtr
        }
        (*(*ctxt).input).cur = b"\x00" as *const u8 as *const libc::c_char as *mut xmlChar;
...
pub unsafe fn xmlSAXParseDTD(
    ...
    if input.is_null() {
        ...
        if !systemIdCanonic.is_null() {
            xmlFree.expect("non-null function pointer")(systemIdCanonic as *mut libc::c_void);
        }
        return 0 as xmlDtdPtr;
    }
    /*
            ...
            if (*node).type_0 as libc::c_uint == XML_ELEMENT_NODE as libc::c_int as libc::c_uint {
                        ...
                        == XML_ELEMENT_NODE as libc::c_int as libc::c_uint
                {
                    ...
                    while !ns.is_null() {
                        if !(*ctxt).dict.is_null() {
                            iprefix =
                                xmlDictLookup((*ctxt).dict, (*ns).prefix, -(1 as libc::c_int));
...
unsafe fn xmlCreateEntityParserCtxtInternal(
    ...
    if uri.is_null() {
        inputStream =
            xmlLoadExternalEntity(URL as *mut libc::c_char, ID as *mut libc::c_char, ctxt);
        if inputStream.is_null() {
            xmlFreeParserCtxt(ctxt);
            return 0 as xmlParserCtxtPtr;
        }
        ...
        inputStream =
            xmlLoadExternalEntity(uri as *mut libc::c_char, ID as *mut libc::c_char, ctxt);
        if inputStream.is_null() {
            xmlFree.expect("non-null function pointer")(uri as *mut libc::c_void);
            xmlFreeParserCtxt(ctxt);
            return 0 as xmlParserCtxtPtr;
...
pub unsafe fn xmlSetupParserForBuffer(
    ...
    xmlClearParserCtxt(ctxt);
    if !filename.is_null() {
        (*input).filename = xmlCanonicPath(filename as *const xmlChar) as *mut libc::c_char
    }
    (*input).base = buffer;
    (*input).cur = buffer;
    (*input).end = &*buffer
        .offset(
            (xmlStrlen as unsafe extern "C" fn(_: *const xmlChar) -> libc::c_int)(buffer) as isize,
        ) as *const xmlChar;
